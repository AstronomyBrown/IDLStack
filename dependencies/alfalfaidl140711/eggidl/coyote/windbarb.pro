...
;
; Copyright © 2003-2004 Fanning Software Consulting
;
...
;      dr  = (wdirection[j] + map_rotation) * !DtoR       ; Wrong direction, 8 June 2004.
      dr  = (wdirection[j] + 180. + map_rotation) * !DtoR ; Rotated to correct (!?) direction.
      ...
      x1 = (xx[j] + sindr * sr) > clip[0]
      y1 = (yy[j] + cosdr * sr * aspect) > clip[1]
      x2 = (x1 + sindr * staff_len) < clip[2]
      y2 = (y1 + cosdr * staff_len * aspect) < clip[3]
      IF Keyword_Set(station) THEN StationPlot, x[j], y[j], Radius=sr, Color=color[j]
      PLOTS, [x1, x2], [y1,y2], /Normal, Color=FSC_Color(color[j]), Clip=clip
      ...
      FOR i = 1, num10 DO BEGIN
         ...
         PLOTS, [x1, x2, x3], [y1, y2, y3], /Normal, Color=FSC_Color(color[j]), Clip=clip
         x1 = x2
         y1 = y2
      ENDFOR
      ...
      FOR i = 1, num50 DO BEGIN
         ...
         IF y2 LT clip[1] OR y2 GT clip[3] THEN CONTINUE
         PLOTS, [x2, x0], [y2, y0], /Normal, Color=FSC_Color(color[j]), Clip=clip
