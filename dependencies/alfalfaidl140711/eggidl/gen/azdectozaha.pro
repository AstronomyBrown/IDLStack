;--------------------------------------------------------------------------------------------------
;+
;NAME: azdectozaha - compute za and hour angle given the az and dec of source
...
;
;----------------------------------------------------------------------
;   substitute 1-sin^2(c) for cos^2(c) --> [1 - sin^2(c)[1-cos^2(A)]]=[1-sin^2(c)sin^2(A)]
;
;cos(b)=[cos(a)cos(c) - sin(a)cos(C)sin(c)cos(A)]
;        ------------------------------------------
;           [1-sin^2(c)sin^2(A)]
...
;
;  HISTORY:
;  6Apr2010  - Phil sends original version 
;              Betsey notes double divide by 15; corrects
;                     adds more decimal places in output
;  7Apr2010  - Martha adds some more detail
;
...
;
        ...
        if keyword_set(verbose) then begin
                ...
                ln=string(format=$
                    '("srcAz:",f7.2,"  decD:",f7.3,"  za:",f6.2,"  ha:",f6.2," mins    PAd:",f7.2)',$
;                az[i]-180.,dec[i],za[i],ha[i]/15.,asin((sinaz[i]*coslat)/cosDec[i])*!radeg)
                 az[i]-180.,dec[i],za[i],ha[i]*60.,asin((sinaz[i]*coslat)/cosDec[i])*!radeg)
                print,ln
                endfor
        endif
        return,1
