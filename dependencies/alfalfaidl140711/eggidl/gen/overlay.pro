...
;
;CALLING SEQUENCE:
;        overlay,grid,znew,rahr,decdeg,c_levels,xmin,xmax,ymin,ymax,img	
;
;OUTPUT:
...
;   Directory reference changed by mph 2013
;
;-

PRO OVERLAY,grid,znew,rahr,decdeg,c_levels,xmin,xmax,ymin,ymax,img
...
contour, znew, rahr,decdeg, xstyle=1, ystyle=1,xrange=[xmax,xmin],yrange=[ymin,ymax],levels=c_levels,xtitle='!6 Right Ascension (J2000)',ytitle='Declination (J2000)',background=255,color=1,/nodata,charsize=1.5,xtick_get=xaxislabel, ytick_get=yaxislabel
erase,color=255
...
;slightly larger than min(abs(imgrexizedimg)). For example, I decided on:
;tvscl,imgresizedimg>(-980),PX[0],PY[0]
;while before inverting the image, min(imgresizedimg)=920
;Adjust below accordingly:

tvscl,imgresizedimg,PX[0]+60,PY[0]+60
...
oplot,[0.,xmax-.01],[0.,ymin+.15],psym=8,symsize=6.6,color=1
xyouts,xmax-.01,ymin+.1,'Beam',color=1,charsize=1.7

;Plot: the position of the plot may need to be moved to match the image.
;for example:
;position=[PX[0],PY[0],PX[0]+SZ[1]-1,PY[0]+SZ[2]-1]
contour, znew, rahr,decdeg, xstyle=1, ystyle=1,position=[PX[0]+60,PY[0]+60,PX[0]+SZ[1]+59,PY[0]+SZ[2]+59],xrange=[xmax,xmin],yrange=[ymin,ymax],xtickname=xlabels,ytickname=ylabels,xtitle='!6 Right Ascension (hour)',ytitle='Declination (degree)',background=255,color=1,/noerase,/device,charsize=1.5
