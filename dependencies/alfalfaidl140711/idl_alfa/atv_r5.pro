...
;
;       This version is 1.5, last modified 11 Aug 2004.
;
...
;-
;
;       Fall 2005 - B. Kent - modifed to allow nstrip and mstruct to be updated
;                             even if atv is already xregistered
;     
;       January 18,2006  B. Kent - modifed atv_getdss procedure to fix colortable problem
;                                  added atv_getned to work with
;                                  nedquery.pro
;                                - changed rowplot procedure such that
;                                  all three plot on the same scale,
;                                  and edge effect crashes are taken
;                                  care of  
;----------------------------------------------------------------------
;        atv startup, initialization, and shutdown routines
;----------------------------------------------------------------------

pro atv_initcommon, mstruct=mstruct, nstrip=nstrip
...
if (n_elements(mstruct) eq 0) then mstruct=0
...
state = {                   $
          ...
          mstruct: mstruct, $            ; m type structure from ALFALFA surveys 
          nstrip: nstrip $               ; current strip as plotted by inspect procedure
          }


print, 'State struct says', state.nstrip
...
pro atv_startup, mstruct=mstruct, nstrip=nstrip
...
; Initialize the common blocks - modifed B. Kent May 2005
atv_initcommon, mstruct=mstruct, nstrip=nstrip
...
common atv_state
common atv_images
common atv_myplot   ;RG ADDED THIS LINE

state.lineplot_size[0]=900   ;Added B. Kent   May 2005
state.lineplot_size[1]=600   ;Added B. Kent   May 2005
...
;....................
;RG ADDED THIS SECTION
nrow = state.coord[1]
ncol = state.coord[0]
strow = string(nrow,format='(i4)')
stcol = string(ncol,format='(i4)')
pixra = rah[nrow]
pixdec=decd
radec=ADSTRING(pixra*15.,pixdec)
frq_0 = RF_frq-(nchn/2)*delta_frq
pixfrq = RF_frq + (ncol-(nchn/2+1))*delta_frq
pixvel = (1420.4058/pixfrq-1.)*2.99792458e5-vel_heliodop[nrow]
;........................


; ADDED HOR LINE TO RESET LINE PLOT LIMITS 04NOV16
;plot, main_image[*, state.coord[1]], $
;  xst = 3, yst = 3, psym = 10, $
;  title = strcompress('Plot of row ' + $
;                      string(state.coord[1])), $
;  xtitle = 'Column', $
;  ytitle = 'Pixel Value', color = 7
;RG ADDED FOLLOWING 2 LINES
;  xyouts,0.2,0.85,'RA, Dec ='+radec,charsize=1.3,/normal
;  xyouts,0.2,0.80,'Frq='+strtrim(pixfrq,2)+'  Vel='+strtrim(round(pixvel),2),$
;       charsize=1.3,/normal 

;BK ADDED EVEN MORE LINES!

;June 26, 2005 - Fixed BK - now makes three separate plots

device, decomposed=1

!p.multi=[0,1,3]

avgpol=(state.mstruct[0,state.coord[1],state.nstrip].d+state.mstruct[1,state.coord[1],state.nstrip].d)/2.0


;Added by B. Kent January 18, 2006 to protect against edge clicking
hormin=STATE.COORD[0]-400
hormax=state.coord[0]+400

if (hormin lt 0) then hormin=0
if (hormax gt n_elements(state.mstruct[0,0,0].d)-1) then hormax=n_elements(state.mstruct[0,0,0].d)-1

HOR,hormin, hormax



;Use the next lines to keep all three plots on the same scale -
;B. Kent January 18, 2006
minver=min([min(state.mstruct[0,state.coord[1], state.nstrip].d[hormin:hormax]), $
           min(state.mstruct[1,state.coord[1], state.nstrip].d[hormin:hormax]), $
           min(avgpol[hormin:hormax])])
maxver=max([max(state.mstruct[0,state.coord[1], state.nstrip].d[hormin:hormax]), $
           max(state.mstruct[1,state.coord[1], state.nstrip].d[hormin:hormax]), $
           max(avgpol[hormin:hormax])])

ver, minver, maxver

plot, state.mstruct[0,state.coord[1], state.nstrip].d, $
       xst = 3, yst = 3, psym=10, $
       title = strcompress('Plot of row ' + $
                      string(state.coord[1])), $
       color = 'FFFFFF'XL, charsize=1.7  ; White

oplot,state.mstruct[0,state.coord[1], state.nstrip].d, psym=10,  color = '0000FF'XL  ; Red for pol A

plot, state.mstruct[1,state.coord[1], state.nstrip].d, $
       xst = 3, yst = 3, psym=10, $
       ytitle='Intensity', $
       color = 'FFFFFF'XL, charsize=1.7  ; White

oplot, state.mstruct[1,state.coord[1], state.nstrip].d, psym=10,  color = '00FF00'XL ; Green for pol B




plot, avgpol, xst=3, yst=3, psym=10, xtitle='Channel Number', color = 'FFFFFF'XL, charsize=1.7  ; White for avg

delvarx, avgpol


;ra=state.mstruct[0,state.coord[1], state.nstrip].rahr
;dec=state.mstruct[0,state.coord[1],state.nstrip].decdeg

print, 'Row plot says ', state.nstrip

;radec=ADSTRING(ra*15.,dec)

  xyouts,0.10,0.90,'RA, Dec ='+radec,charsize=1.3,/normal
  xyouts,0.10,0.85,'Frq='+strtrim(pixfrq,2)+'  Vel='+strtrim(round(pixvel),2),$
       charsize=1.3,/normal 

  xyouts,0.85,0.90, 'PolA', color = '0000FF'XL, charsize=2.0, /normal ; Red for pol A
  xyouts,0.85,0.85, 'PolB', color = '00FF00'XL, charsize=2.0, /normal ; Green for pol B
  xyouts,0.85,0.80, 'Avg' , color = 'FFFFFF'XL, charsize=2.0, /normal ; White for avg
...
;Print on screen ccoords of pixel when prompting via 'x'
...
pixdec=decd
radec=ADSTRING(pixra*15.,pixdec)
pixram=(pixra-floor(pixra))*60.
pixras=(pixram-floor(pixram))*60.
pixdm=(pixdec-floor(pixdec))*60.
pixds=(pixdm-floor(pixdm))*60.
radec=string(floor(pixra),floor(pixram),pixras, $
             floor(pixdec),floor(pixdm),floor(pixds), $
             format='(i2.2,1x,i2.2,f5.1," +",3(i2.2,1x))')
frq_0 = RF_frq-(nchn/2)*delta_frq
pixfrq = RF_frq + (ncol-(nchn/2+1))*delta_frq
pixvel = (1420.4058/pixfrq-1.)*2.99792458e5-vel_heliodop[nrow]

Print,nrow,ncol,radec,pixvel,format='("Pix:",2i5," at:",a21,",  v:",f7.0)'
...
pixdm=(pixdec-floor(pixdec))*60.
pixds=(pixdm-floor(pixdm))*60.
radec_noblanks=string(floor(pixra),floor(pixram),round(pixras), $
                      floor(pixdec),floor(pixdm),floor(pixds), $
                      format='(3i2.2,"+",3i2.2)')
frq_0 = RF_frq-(nchn/2)*delta_frq
pixfrq = RF_frq + (ncol-(nchn/2+1))*delta_frq
...
xchnarr = lindgen(nchn)
xfrqarr = frq_0 +xchnarr*delta_frq
xvelarr = (1420.4058/xfrqarr -1.)*2.99792458e5
yarr = reform(main_image[*, nrow])
y7 = reform(mask7[*,state.coord[1]])

xarr = xchnarr
xtitle = 'Channel Nr.'
toggle7 = 1.
overy = y7
ver,-0.1,0.3  ;default vertical scale
plot,xarr,yarr,title='Spectral profile  Rec='+strow,xtitle=xtitle
oplot,xarr,overy,color=5,thick=2
xyouts,0.2,0.90,'Ch='+strtrim(ncol,2)+'  Rec='+strtrim(nrow,2),$
       charsize=1.3,/normal
xyouts,0.2,0.85,'RA, Dec ='+radec,charsize=1.3,/normal
xyouts,0.2,0.80,'Frq='+strtrim(pixfrq,2)+'  Vel='+strtrim(round(pixvel),2),$
       charsize=1.3,/normal 
 
;THIS SHOULD BE DONE USING A WIDGET....
 
while 1 do begin
  print,'_______________________________________________________'
  print,''
  print,'KEY  ARGS      FUNCTION'
  print,'x  xax     : select x-axis'
  print,'             xax=0 ch nr'
  print,'             xax=1 freq'
  print,'             xax=2 vel'
  print,'h  h1  h2  : set horiz scale and replot'
  print,'             (set h1,h2 to 0,0 to free scale)'
  print,'v  v1  v2  : set vert scale and replot'
  print,'             (set v1,v2 to 0,0 to free scale)'
  print,'s  gch     : gauss_smooth over gch channels'
  print,'m          : measure the feature'
  print,'7          : switches on and off the overplot of strip 7'
  print,'p          : make ps file of plot and write to specdet'
  print,'q          : quit myplot'
  line='string'
  read,'?',line
  line=strtrim(strlowcase(line),1) ; convrt to lowercase & remv leading blnks
  itemp=strpos(line,' ') ; find char nr w 1ast occurrence of blnk
  len=strlen(line) ; nr of chars in line
  if (itemp ne -1) and((itemp+1) le len) then begin
    lineargs=strmid(line,itemp+1,len-(itemp)) ; contains args, e.g. 'h1 h2'
  endif else begin
    lineargs=''
  endelse
  ...
  case strmid(line,0,1) of
    ...
    'x': begin
         reads,lineargs,xax
         if (xax eq 0) then begin
           xarr=xchnarr
           xtitle='Channel Nr'
         endif 
         if (xax eq 1) then begin
           xarr=xfrqarr
           xtitle='Frequency (MHz)'
         endif
         if (xax eq 2) then begin
           xarr=xvelarr
           xtitle='cz (km/s)'
         endif              
         end
    ...
    'h': begin
         ...
         hor,h1,h2
         if (h1 eq 0 and h2 eq 0) then hor
         end

    'v': begin
         reads,lineargs,v1,v2
         ver,v1,v2
         if (v1 eq 0 and v2 eq 0.) then ver
         end

    's': begin
         reads,lineargs,gch
         gauss_kern = fltarr(nrec)
         sigma=0.4247*gch
         three_s=round(3.*sigma)
         sum = fltarr(nchn)
         for i=nrow-three_s,nrow+three_s do begin
           gauss_kern[i]=exp(-((i-nrow)/sigma)^2/2.)
           sum = reform(main_image[*,i])*gauss_kern[i] + sum
         endfor
         yarr = sum/(2.500*sigma)
         end

    'm': begin
         print,'Right click left side of the feature'
         cp,x=x,y=y
         x1=round(x)
         nchnlo = x1
         wait,0.5
         print,'Right click right side of the feature'
         cp,x=x,y=y
         x2=round(x)
         nchnhi = x2
         flag,x1
         flag,x2
         tdv = total(yarr(x1:x2))*abs(xvelarr(x1)-xvelarr(x1+1))
         meant = mean(yarr(x1:x2))
         width=tdv/meant
         vcentroid=total(yarr[x1:x2]*xvelarr[x1:x2])/total(yarr[x1:x2])
         wait,0.5
         print,'Right click left side of region for rms estimate'
         cp,x=x,y=y
         x1=round(x)
         wait,0.5
         print,'Right click right side of region for rms estimate'
         cp,x=x,y=y
         x2=round(x)
         rms=stddev(yarr[x1:x2])
         peak=max(yarr[x1:x2])
         wsmo = width
         if (wsmo gt 100.) then wsmo=100.  ; assumes rms doesn't decrease by avging > 100 kms
         ston= (meant/rms)*sqrt(wsmo/20.)  ; inspected profile assumed to be han=5 smoothed
         print,'Integrated flux: ',tdv
         print,'Mean flux:       ',meant
         print,'Centroid vel:    ',vcentroid
         print,'Width:           ',width
         print,'rms:             ',rms
    ...
    '7': begin
          toggle7=toggle7*(-1)
          overy = y7
          if(toggle7 gt 0) then  overy = fltarr(nchn)
         end  
    ...
    'p': begin
         ...
         read,'?',filename
         if (filename eq '') then filename='s'+strtrim(radec_noblanks,2)+'.ps'
         set_plot,'PS'
         device,filename=filename
           plot,xarr,yarr,title='Spectral profile  Rec='+strow,ystyle=1,xtitle=xtitle
           oplot,xarr,overy,color=5,thick=2
           xyouts,0.2,0.90,'Ch='+strtrim(ncol,2)+'  Rec='+strtrim(nrow,2),$
                  charsize=1.3,/normal
           xyouts,0.2,0.85,'RA, Dec ='+radec,charsize=1.3,/normal
         ...
         device,/close_file    
         set_plot,'X'
         print, 'Enter   nsmo,     detcode'
         print, ' -----------------------'
         print, '      1=no smo   1=real'
         print, '      3=han3     2=good' 
         print, '      5=han5     3=fair' 
         print, '      7=han7     4=marginal' 
         print, '                 5=poor' 
         print, '                 6=confused/blend' 
         print, '                 7=corrupted'
         print, '                99=non det'
         read,sp_smo,detcode 
         print,'Add comments to the parameters file:'
         comments=''
         read,comments
         openw,lun,'spec.parms',/append,/get_lun
         printf,lun,strtrim(radec_noblanks,2),ns,np,vcentroid,tdv,meant,width,rms,ston, $
                detcode,comments,format='(a13,2i3,f8.0,f9.2,f7.3,f8.0,f7.3,f7.2,1x,i2,1x,a)'
         free_lun,lun
;         openw,lun,strtrim(radec_noblanks,2)+'.sdat',/get_lun
;         for i=0,nchn-1 do begin
;           printf,lun,xvelarr[i],xfrqarr[i],yarr[i],format='(f8.1,f10.3,f10.4)'
;         endfor
;         free_lun,lun
         
;        GO WRITE THE SPECDET FILE
         make_specdet,spec1template
         if (specappend eq 0) then begin  ; start specdet from scratch
           specdet= replicate({spec1template},1)
           spec1  = replicate({spec1template},1)
         endif else begin
           spec1  = replicate({spec1template},1)
         endelse
         spec1.name = strtrim(radec_noblanks,2)
         spec1.rahr = d_st[0,nrow,ns].rahr
         spec1.decd = d_st[0,nrow,ns].decdeg
         spec1.strip = ns
         spec1.nrec = nrow
         spec1.npol = np
         spec1.d_file[0] = d_st[0,0,0].h.proc.iar[0]
         spec1.d_file[1] = d_st[0,0,0].h.proc.iar[1]
         spec1.d_file[2] = d_st[0,0,0].h.proc.iar[2]
         spec1.cen_f = d_st[0,0,0].h.dop.freqbcrest*d_st[0,0,0].h.dop.factor
         spec1.cen_v = d_st[0,0,0].h.dop.velorz
         spec1.restfrq = 1420.4058
         spec1.cen_ch = nchn/2 + 1
         spec1.ch_fwidth = 0.02441406  ; will eventually compute it from hdr
         spec1.heliovelproj = d_st[0,nrow,ns].h.pnt.r.heliovelproj*2.997922458e5
         spec1.nchlo = nchnlo
         spec1.nchhi = nchnhi
         spec1.vcen[0]  = vcentroid
         spec1.width[0] = width
         spec1.mean_int = meant
         spec1.peak_int = peak
         spec1.flux_int = tdv
         spec1.rms = rms
         spec1.ston[0] = ston
         spec1.smo = sp_smo
         spec1.detcode[0] = detcode
         spec1.comments = comments
         spec1.date_of_red = systime()
         spec1.mjd = d_st[0,nrow,ns].hf.mjd_obs  ; = jd-2400000.5
         spec1.data[*,0] = xvelarr
         spec1.data[*,2] = yarr
         spec1.data[*,3] = d_st[np,nrow,7].d

         if (specappend eq 0) then begin
           specdet[0] = spec1
           specappend = 1
         endif else begin
           specdet = [specdet,spec1]
         endelse  

         goto, done
         end

    else: Print,'Bad input: must enter x, h, v, 7, p or q'
...
;--------------------------------------------------------------------
;Added by Brian Kent, June 29, 2005, Cornell University
;Press g key and queries for a DSS/Sloan image image
...
rahr=state.mstruct[0,state.coord[1], state.nstrip].rahr
decdeg=state.mstruct[0,state.coord[1],state.nstrip].decdeg

getdss, rahr, decdeg    ;No keyword passed - default is 6 arcminutes (faster loading)
                        ;Changable within the GUI
...
;--------------------------------------------------------------------
;Added by Brian Kent, January 18, 2005, Cornell University
...
widget_control, state.base_ID, hourglass=1

nedquery, rahr*15.0, decdeg,10.0, numberinfo=numberinfo, string_array=string_array

h=['Searched within 10 arcminutes of RA= '+strcompress(rahr, /remove_all)+'   Dec= '+strcompress(decdeg, /remove_all), $
   numberinfo, '  Name 			         RA(J2000)   DEC(J2000)	TYPE      VEL    Z	   SKY DIST(arcmin)',string_array]


if (not (xregistered('atv_getned', /noshow))) then begin

helptitle = strcompress('NED Results')
    ...
    xmanager, 'atv_getned', help_base, /no_block
    
endif



widget_control, state.base_ID, hourglass=0
...
;Added BK 2005 - may be taken away 

;Fixed BK June 26, 2005 - now makes three separate plots 

device, decomposed=1

!p.multi=[0,1,3]
plot, state.mstruct[0, *, state.nstrip].d[state.coord[0]], $
      xst = 3, yst = 3, psym=10, $
      title = strcompress('Plot of row ' + $
                      string(state.coord[1])), $
      color = 'FFFFFF'XL, charsize=1.7  ;To set axes as white

oplot, state.mstruct[0,*, state.nstrip].d[state.coord[0]], $
       psym=10, color = '0000FF'XL; Red for pol A

plot, state.mstruct[1,*, state.nstrip].d[state.coord[0]], $
       xst = 3, yst = 3, psym=10, ytitle='Intensity', $
       color = 'FFFFFF'XL, charsize=1.7  ;To set axes as white

oplot, state.mstruct[1,*, state.nstrip].d[state.coord[0]], $
       psym=10, color = '00FF00'XL ; Green for pol B


avgpol=(state.mstruct[0,*,state.nstrip].d[state.coord[0]]+state.mstruct[1,*,state.nstrip].d[state.coord[0]])/2.0

plot, avgpol, psym=10, color = 'FFFFFF'XL, charsize=1.7, xtitle='Channel Number'   ; White for avg
...
common atv_state
common atv_images
common atv_myplot         ; LINE ADDED BY RG 04Nov17, defined in inspect
common atv_specdet        ; LINE ADDED BY RG 04Nov30, defined in inspect
...
if (n_elements(nstrip) eq 0) then nstrip=0

if ((xregistered('atv', /noshow))) then begin
    state.mstruct=mstruct
    state.nstrip=nstrip
endif

if (not (xregistered('atv', /noshow))) then begin
   userwindow = !d.window
   atv_startup, mstruct=mstruct, nstrip=nstrip                   ; modifed B. Kent May 2005
   align = 0B     ; align, stretch keywords make no sense if we are
   stretch = 0B   ; just starting up. 
endif

;.............................................
;  SECTION ADDED BY RG ON 04NOV17, TO turn STRUCTURE myplot_parm into common bock
if (n_elements(d_st) eq 0) then begin
  nrec = 600L   ;changed from 900L to 600L by B. Kent July 19, 2005
  nchn = 4096L
  ns   = 0L
  np   = 0L
  rah = dblarr(nrec)
  decd = 0D
  vel_heliodop = dblarr(nrec)
  mask7 = fltarr(nchn,nrec)
  delta_frq = 0D
  delta_ra = 0D
  RF_frq = 0D
endif
if (n_elements(specdet) ne 0) then begin
  if (n_elements(specappend) eq 0) then specappend=1
  if (specappend ne 0) then specappend=1
endif else begin
  specappend=0
endelse
