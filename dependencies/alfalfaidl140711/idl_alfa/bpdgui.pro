...
;       Apr 19 2005  Interp Reg module updated
;       Apr 25 2005  New version added to egg directory
;       Apr 26 2005  Added 'Apply to All' option in Interp_edit
;       May 23 2005  Changed CRAT fit to default option in widget
;-
;-----------------------------------------------------------------------
...
common bpdgui_info
   ...
   if(filename ne '') then begin
	...
	widget_control, info.unprocesseddisplaylist, set_value=info.unprocessedlist[0:info.numunprocessedlist-1]
        widget_control, info.unprocesseddisplaylist, set_uvalue=info.unprocessedlist[0:info.numunprocessedlist-1]      

	widget_control, info.unprocessedlistlabel, set_value=strcompress(ifound, /remove_all)+ ' raw files:'
...
; Initalize the default interpolation regions
       
       info.interp_reg[*,0]=3480
       info.interp_reg[*,1]=3520

       listval=strarr(100)
        for i=0,info.numunprocessedlist-1 do begin
            tempstring=''
            for j=0,9 do begin
              tempstring=tempstring+strcompress(info.interp_reg[i,j], /remove_all)+','
          endfor
              listval[i]=tempstring
        endfor

       widget_control, info.interp_reg_list, set_value=listval[0:info.numunprocessedlist-1]
       widget_control, info.interp_reg_list, set_uvalue=listval[0:info.numunprocessedlist-1]
       widget_control, info.interp_reg_list, set_list_select=0
   ...
   common bpdgui_info
    ...
    if(filenamelist eq '' or ncalibfilename eq '') then begin
	  ...
	  if (info.calsessionselect eq 0) then calsessionselect='Start New Calsession, File:'
          if (info.cratcurrent eq 0) then cratcurrent='Use CRAT fit'
          if (info.poswhatselect eq 0) then poswhatselect='Start New pos, File:'
	  ...
	  ;Find any flagged boards       
		...
		restore, calsessionfilename
          ...
          if (poswhatselect eq 'Append to pos, File:') then begin
		poswhat='append'
		widget_control, info.usecurrentposwhatfile, get_value=poswhatfilename
		restore, poswhatfilename
	  endif

          if (poswhatselect eq 'Start New pos, File:') then poswhat='new'
	...
	for ifile=0, info.numunprocessedlist-1 do begin   
	  widget_control, info.unprocesseddisplaylist, set_list_select=ifile
          widget_control, info.interp_reg_list, set_list_select=ifile
	  ...
	  widget_control, event.top, hourglass=0
  	  ...
  	  widget_control,info.filenamelist, get_uvalue=dptr
          ...
          interp_reg_temp=reform(info.interp_reg[ifile, *])
 
          interp_reg=interp_reg_temp[where(interp_reg_temp ne 0)]
          print, 'Interpolation region used: '+strcompress(interp_reg)
          ;calls bpd - written by RG
           bpd, d, ncalib, brdstat=brdstat, calwhat=calwhat, poswhat=poswhat, $
                force_interp=1, interp_reg=interp_reg, $
		calsession=calsession, runpos=pos, fileout=fileout      

	  ;write calsession and runpos(now rename as pos) to file - start new OR append old
...
;                pos=runpos
		save, pos, filename=poswhatfilename
	   endif
...
;                pos=runpos
	  ...
	  widget_control, info.processeddisplaylist, set_value=info.processedlist[0:ifile]
          widget_control, info.processeddisplaylist, set_uvalue=info.processedlist[0:ifile]
   ...
   if(filename ne '') then begin
     ...
     widget_control, info.cregionwindow, get_value=index
       ...
       for i=0,n_elements(chanvals)-2,2 do begin
        ...
        loadct, 11, /silent
        !p.position=[0.01,0.25,0.98,0.75]
        contour, mask, /fill, nlevels=1, title='Beam 0  Pol A', charsize=0.9  ;Default plot
        loadct, 0, /silent
    ...
    if(filenamelist eq '' or ncalibfilename eq '') then begin
     result=dialog_message('Please open an *.list AND ncalib file for processing.', /information)
      ...
      if (not (xregistered('bpdgui_chooseinterp', /noshow))) then begin
       ...
       chooseinterpbase =  widget_base(group_leader = info.baseID, /column, /base_align_right, $
        ...
        info.chooseinterpdraw=widget_draw(chooseinterpbase, xsize=800, ysize=400, uvalue='chooseinterp_draw', /button_events)
	 ...
	 xscalebase=widget_base(scalebase, column=4, /align_left, /grid_layout)
         label=widget_label(xscalebase, value=' Channels min:')
           info.xmin=widget_text(xscalebase, /editable, xsize=8)
         label=widget_label(xscalebase, value='     Channels max:') 
           info.xmax=widget_text(xscalebase, /editable, xsize=8)
         
         yscalebase=widget_base(scalebase, column=4, /align_left, /grid_layout)
         label=widget_label(yscalebase, value='Intensity min:')
           info.ymin=widget_text(yscalebase, /editable, xsize=8)
         label=widget_label(yscalebase, value='    Intensity max:')
           info.ymax=widget_text(yscalebase, /editable, xsize=8)

         labelbase=widget_base(chooseinterpbase, /column, /base_align_right)
         info.chooseinterpdrawlabel=widget_label(labelbase, $
            value='  Click on the above spectrum to chose the regions to interpolate over, or click Default')
         widget_control, info.unprocesseddisplaylist, get_uvalue=filelist
         file=filelist[info.currentfile]
         label=widget_label(labelbase,value='File: '+file, frame=1)

         
         buttonbase=widget_base(bottombase, column=5, /base_align_right, /grid_layout)
         chooseinterp_zoomin=widget_button(bottombase, value= 'Zoom In', uvalue='chooseinterp_zoomin')
      ...
      info.totalspectrum=fltarr(4096)
	...
	info.totalspectrum+=total(reform(d[0,*,i].d),2)
     ...
     ;             -300.0, max(info.totalspectrum)
      ...
      plot, info.totalspectrum, xtitle='Channel', ytitle='Intensity of totaled spectra', charsize=1.2, ticklen=0.01
         ...
         if (n_elements(where(info.interp_reg[info.currentfile,*] ne 0)) mod 2 eq 0) then begin

           widget_control, info.chooseinterpdraw, get_value=index
           wset, index     
           !p.position=[0.1,0.1,0.97,0.97]
           chanvals=info.interp_reg[info.currentfile,where(info.interp_reg[info.currentfile,*] ne 0)]
           mask=fltarr(4096,4)
           x=indgen(4096)
...
case uvalue of
    'chooseinterp_done': begin
        ...
        ;Update the list in the main window and for uvalues
        listval=strarr(100)
        for i=0,info.numunprocessedlist-1 do begin
            tempstring=''
            for j=0,9 do begin
        ...
        widget_control, info.interp_reg_list, set_value=listval[0:info.numunprocessedlist-1] 
        widget_control, info.interp_reg_list, set_uvalue=listval[0:info.numunprocessedlist-1]
        widget_control, info.interp_reg_list, set_list_select=info.currentfile
    ...
    'chooseinterp_draw': begin
       if (event.press gt 0) then begin
         ...
         loc=where(info.interp_reg[info.currentfile,*] eq 0)

         xy=convert_coord(event.x, event.y, /device, /to_data)
          
         info.interp_reg[info.currentfile,loc[0]]=xy[0]
...
;     print, xmin, xmax, ymin, ymax
    ...
    'chooseinterp_default' : begin
     ...
     info.interp_reg[info.currentfile,0]=3480
     info.interp_reg[info.currentfile,1]=3520
     info.interp_reg[info.currentfile,2:9]=0
     print, 'Default set to Galactic HI region channels: 3480,3520'     
     ...
     widget_control, event.top, /destroy
      ...
      'chooseinterp_reset' : begin
         ...
         info.interp_reg[info.currentfile,*]=intarr(10)
         plot, info.totalspectrum, xtitle='Channel', ytitle='Intensity of totaled spectra', charsize=1.2, ticklen=0.01
...
;-------------------------------------------------------------------------------------
pro bpdgui_selectfile, event

common bpdgui_info

widget_control, event.id, get_uvalue=filelist

  selectedfile=filelist[event.index]
  
  widget_control, event.id, set_list_select=event.index
  widget_control, info.interp_reg_list, set_list_select=event.index

widget_control, event.top, hourglass=1
  bpdgui_opensavfile, selectedfile
widget_control, event.top, hourglass=0

info.currentfile=event.index
info.currentreg=event.index
...
if (not (xregistered('bpdgui_interp_edit', /noshow))) then begin

title = strcompress('INTERP REG EDIT WINDOW')

    interp_edit_base =  widget_base(group_leader = info.baseID, $
                             /column, $
                             /base_align_right, $
                             title = title, $
                             uvalue = 'interp_edit_base')

     label=widget_label(interp_edit_base, value='Interp Region for file: '+selectedfile)

     interp_edit_row_base=widget_base(interp_edit_base, row=1, /align_left)
	   for i=0,9 do info.interp_edit[i]=widget_text(interp_edit_row_base, xsize=10, /editable)

    interp_buttons=widget_base(interp_edit_base, column=2, /align_right, /grid_layout)
    interp_edit_all = widget_button(interp_buttons,value= ' Apply to All', uvalue='interp_edit_all')

    interp_edit_done = widget_button(interp_buttons, value = ' Done ', uvalue = 'interp_edit_done')

    

    widget_control, interp_edit_base, /realize
    xmanager, 'bpdgui_interp_edit', interp_edit_base, /no_block

    for i=0,9 do begin
        widget_control, info.interp_edit[i], set_value=strcompress(info.interp_reg[event.index,i])
    endfor
...
pro bpdgui_interp_edit_event, event
...
case uvalue of
    'interp_edit_done': begin

        for i=0,9 do begin
            widget_control, info.interp_edit[i], get_value=value
            info.interp_reg[info.currentreg,i]=uint(value)            
        endfor
        ...
        widget_control, info.interp_reg_list, set_list_select=info.currentfile
        info.currentreg=info.currentfile

        widget_control, event.top, /destroy

    end

    'interp_edit_all': begin

        for i=0,9 do begin
            widget_control, info.interp_edit[i], get_value=value
            info.interp_reg[*,i]=uint(value)            
        endfor
...
h[i] =  'BPDGUI HELP'
i = i + 1
h[i] =  'Last update:  April 26, 2005,   B. Kent, Cornell U.'
i = i + 1
h[i] =  ''
i = i + 1
...
i = i + 1
h[i] =  'Interp Regions:         Interolate in region of galactic HI'
h[i] =  ''
i = i + 1
...
tlb=widget_base(column=1, title='ALFALFA Bandpass Calibration', $
                  tlb_frame_attr=1, xsize=1025, ysize=690)
...
cratbase=widget_base(leftbase, column=1, /align_left, /exclusive)
   ...
   widget_control, info.usecratfit, Set_Button=1
...
poswhatbase=widget_base(leftbase, row=1, /align_left)
   poswhatselectbase=widget_base(poswhatbase, column=1, /align_left, /exclusive)
      usecurrentposwhat=widget_button(poswhatselectbase, $
          value='Append to pos, File:', uvalue='poswhatselect', event_pro='bpdgui_buttonselect')
      newposwhat=widget_button(poswhatselectbase, $
          value='Start New pos, File:', uvalue='poswhatselect', event_pro='bpdgui_buttonselect')
...
ncalibcalmaskbase=widget_base(rightbasetop, column=1, /align_left, frame=1)
   ncalibcalmaskheader=widget_base(ncalibcalmaskbase, row=1, /align_left)
    ...
    cregionbase=widget_base(ncalibcalmaskbase, column=1, /align_center, frame=1)
       info.cregionwindow=widget_draw(cregionbase, xsize=225, ysize=75)
...
bottomrightbase=widget_base(rightbase, row=1, /align_left)

 filelistbase=widget_base(bottomrightbase, column=1, /align_left)
   info.unprocessedlistlabel=widget_label(filelistbase, value='   Raw files:', /align_center)
   info.unprocesseddisplaylist=widget_list(filelistbase, xsize=28, ysize=9, value=['',''], event_pro='bpdgui_selectfile')

 processedlistbase=widget_base(bottomrightbase, column=1, /align_left)
   info.processedlistlabel=widget_label(processedlistbase, value='Processed files:', /align_center)
   info.processeddisplaylist=widget_list(processedlistbase, xsize=28, ysize=9, value=['',''])

 interp_reg_base=widget_base(bottomrightbase, column=1, /align_left)
   label=widget_label(interp_reg_base, value='Interp Regions:', /align_center)
   info.interp_reg_list=widget_list(interp_reg_base, xsize=28, ysize=9, value=['',''], event_pro='bpdgui_interp_edit')
      ;Note - will be initialized when filelist is opened
