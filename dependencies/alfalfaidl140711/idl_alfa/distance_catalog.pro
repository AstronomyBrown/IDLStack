...
;		that one is given
;	92 - the object does not have a primary distance measurement, and is assigned to one of A,B,M,W,W' in Virgo
;		so the distance to that cluster or cloud is given.
;
...
;	Fixed bugs when using list of AGC numbers                             G. Hallenbeck March, 2011
;	Added support for Virgo Cluster assignments                           G. Hallenbeck April, 2011
;	Edited output of .agc file so that it treats csv and agc inputs separately	M. Jones June, 2014
;	Changed way structures are saved to .sav files                        G. Hallenbeck June, 2014
;	Changed l821 from 'vel = velgrp_cmb' to 'cmbvel = velgrp_cmb'	      M. Jones June, 2014
;
;
;
;
...
; Now, convert everything into a standardized format for this program
data = {AGCnum:data.agcnumber[where(data.v21 ne 0)], ra:ra, dec:dec, vel:data.v21[where(data.v21 ne 0)], $
  cmbvel:fltarr(n_elements(ra)), dist:dblarr(n_elements(ra)), edist:dblarr(n_elements(ra)), f1:intarr(n_elements(ra)), $
  f2:intarr(n_elements(ra))}
...
for i=0L, n_elements(data.ra)-1 do begin
	...
	if (data.AGCnum[i] ne -1) then begin	
		...
		; Check for Virgo membership
		virgodistance_check, AGCnum, flag, vcc_entry
		if (flag ne 0) then begin
			data.dist[i]  = vcc_entry.dist
			data.edist[i] = vcc_entry.edist
			data.f1[i]    = -1
			data.f2[i]    = vcc_entry.flag
			continue
		endif
...
if (type eq AGC or type eq CSV) then begin
	...
	; Make a header
	if type eq AGC then begin
		printf, out, '#AGCNUM     Coords   HelVel     Dist    EDist F1 F2'
	endif else begin
		printf, out, '#AGCNUM     RA	Dec   HelVel     Dist    EDist F1 F2'
	endelse
	...
	for i=0L, n_elements(data.ra)-1 do begin
		...
		printf,out, data.AGCnum[i], raw_data.rah[i] , raw_data.ram[i],  raw_data.ras10[i], raw_data.sign[i], $
			...
			printf,out, data.AGCnum[i], data.ra[i] , data.dec[i], data.vel[i],   data.dist[i],     data.edist[i],$
		          	data.f1[i], data.f2[i], $
                  	format='(I6,1X,F11.7,1X,F11.7,1X,I5,1X,F8.2,1X,I2,1X,I2)'
		endelse
	endfor
	...
	data = {agcnum:data.agcnum, ra:data.ra, dec:data.dec, vel:data.vel, cmbvel:data.cmbvel, dist:data.dist, $
			edist:data.edist, f1:data.f1, f2:data.f2, code:data.code, loghimass:logmasses, W50:raw_data.W50, $
			flux:raw_data.flux, fluxerr:raw_data.fluxerr}
...
; Make another structure which will always be called 'dist'
; So that we don't get into weird problems with people renaming things
...
x = execute('save, ' + varname + ', dist, filename=basename+".sav"')
