...
;   It loads in the VCC (Virgo Cluster Catalog) data
;   It loads the distances for each of the VCC subclusters/clouds
;   It cross-references the VCC data with the distances
;
; All of the data is then kept in a common block to be referenced
; by virgodistance_check.
;
PRO virgodistance_load

	common __virgodistances, virgolist

	; distance source flag for distance in VCC
	VIRGO_DIST = 92

	; TESTING PURPOSES ONLY
	; Use the local directories, rather than IDL_ALFA
	common agcshare, agcdir
;	agcdir='/home/hatillo/ghallenbeck/vcc/datafiles/'

	crossfile = agcdir+'vcc_agc.cross'
	distfile  = agcdir+'vcc.dist'

	openr, in, crossfile, /get_lun

	VCC_ENTRIES = 3000
	; Define a template to read the data from the file
	temp = {AGC:0L, other:'', vcc:0, radec:'', member:'', cloud:'', bmag:0.0, vel:0.0, othername:'', type:''}
	vcc = replicate(temp, VCC_ENTRIES)
	line = ''
	...
	while not(eof(in)) do begin
		readf, in, line
		if (strpos(line, '#') ne -1) then continue
		reads, line, temp, format='(I6,1x,A7,2x,I4,1x,A14,1x,A1,1x,A2,1x,F5.2,1x,I5,1x,A8,1x,A20)'
		temp.cloud = strtrim(temp.cloud, 2) ; Get rid of whitespace if group ID is only 1 character
	...
	openr, in, distfile, /get_lun

	DIST_ENTRIES = 100
	; Define a template to read the data from the file
	temp = {cloud:'', dist:0.0, edist:0.0, source:''}	
	dist = replicate(temp, DIST_ENTRIES)
	...
	while not(eof(in)) do begin
		readf, in, line
		if (strpos(line, '#') ne -1) then continue
		reads, line, temp, format='(A2,3x,F7.4,2x,F5.3,1x,A23)'
		temp.cloud = strtrim(temp.cloud, 2) ; Get rid of whitespace if group ID is only 1 character
	...
	; Cross-reference AGC numbers with distances and flags

	temp = {AGC:0L, dist:0.0, edist:0.0, flag:0, cloud:''}
	virgolist = replicate(temp, n_elements(vcc))

	j=0L
	for i=0L, n_elements(vcc)-1 do begin
		entry = (where(vcc[i].cloud eq dist.cloud))[0]
		if (entry eq -1) then continue
		virgolist[j] = {AGC:vcc[i].agc, dist:dist[entry].dist, edist:dist[entry].edist, flag:VIRGO_DIST, $
			cloud:vcc[i].cloud}
		j++
	endfor
	virgolist = virgolist[0:j-1]

END



PRO virgodistance_check, AGC, flag, data

	common __virgodistances

	; distance source flag for distance in VCC
	VIRGO_DIST = 92

	entry = (where(virgolist.agc eq AGC))[0]

	; Galaxy doesn't appear in the Virgo cluster
	if (entry eq -1) then begin
		flag = 0
		data = 0
		return
	endif

	flag = virgolist[entry].flag
	data = virgolist[entry]
