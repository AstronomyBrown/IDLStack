...
;       /KEEPT  set this to allow peaks found with the 't' function to
;               persist from one map to the next
;
;DESCRIPTION:
;
...
; Beam number 7 is a special value that represents a potential box not
; necessarily activated in any beam, but that has known dimensions and is ready
; to be added with 'a' or 'A'.
...
; their locations are indicated by YELLOW numbers across the top of the drift
...
; of the data in that box (e.g. row, column plots, stat). It only
...
; 
; Option "b"
; returns the user to the previous drift, while retaining the BADBOXes
; created in the current drift.
;
; TIPS:
...
;    them or delete them
; 3) If you noticed that a .BADBOX should have been activated or deleted
;    in a given map, use the "b" option to return to that map. This
;    will not affect any changes you made to the current map.
; 4) If necessary, you can always reset it manually after you exit
;    FLAGBB. E.g. you forgot to deactivate
...
;
;  07Jan09 - MH - increased MAXREC to 150000 for AGC 
;
;  10Mar10 - DK 
;            - allows user to define a badbox automatically without
;              having to copy/paste the box corners using the 'q'
;              option, or simply RETURN when drawing a box
;            - added functionality to go back to the previous map
;              with 'b' option
;            - revised DSS Viewer window to decide on more reasonable tick
;              marks, and added display of cursor's RA and DEC
;            - peaks found with 't' no longer persist into the next map
;
;  11Mar10 - Version of 2010 flagbb for Tom 
;            Changed red colors to yellow to help Tom who is RG-colorblind.
;
;  09Dec12 - MH - increased MAXREC to 200000 for AGC 
...
oplot,intarr(nchn),color='FFFF00'XL
...
; CHANGED 10 Mar 2010 by DAK
; this procedure now stores the positions of the box corners in the
; "outBox" variable and returns them to the calling function
; "outBox" returns zero if user cancels; calling function must check
; for this condition before doing anything with contents of "outBox"

pro check_drift,image,outBox,mapwindow=mapwindow
...
print, 'Left click UPPER RIGHT corner of box...'
    ...
    if (xpos2 le xpos1 or ypos2 le ypos1) then begin
        print,'ERROR: The upper right corner of the box must be up and to the right of the lower left corner. Please try again.'
        goto,upper_right
    endif
    wait, 0.5
...
while 1 do begin
  ...
  print,'x    : reject box and exit'
  print,'q    : accept box and quit (default)'
  char=''
  read,char

  if(char eq '') then char='q'

  case char of
    'q': begin
        outBox = [xpos1,ypos1,xpos2,ypos2]
        goto,done
    end
...
;   print,ns,np,format='("Pix boxes thought bad for BEAM",i2," POL",i2)'
   ...
   print,'               - go back to previous map                              (b)'
...
pro flagbby,	 	$;
	...
	MASK=mask,	$; RFI mask (use only if RFIMOD=2
        KEEPT=keept      ; if set, program allows peaks found with 't' to persist to next map

; AFTER BPD, THE HOUSE RECOMMENDS:
;	flagbby,dred,cont_pt,mask,pos,gausav=11,han=3,agc=1
...
; Remove any previous instances of 't'
     if (n_elements(KEEPT) eq 0) then begin
...
;
;  Modify next line to make the badboxes appear yellow rather than red
;
...
;     IF YOU GOT HERE, FIND the AGC GALS TOUCHED BY the DRIFT AND PLOT THEM IN GREEN
   ...
   'e': goto, NextPol
   'b': begin
        if(nn eq n1 and np eq 0) then goto,menu
...
;        endif
   ...
   'm': begin
        ...
        check_drift,IMAGE,boxcorners,mapwindow=mapwindow
        if(n_elements(boxcorners) eq 4) then begin
            badbox[nbox,np,ns,*]=boxcorners
            badbox[nbox, 0, 7,*]=boxcorners
            badbox[nbox, 1, 7,*]=boxcorners
        endif
        wdelete,mapwindow
        goto,display_image
        end
   ...
   'M': begin
        ...
        if(n_elements(boxcorners) eq 4) then begin
            ...
            if (nn lt n2) then begin
                ...
                for nss=nnn1,n2 do begin
                    for npp=0,1 do begin
                        badbox[nbox,npp,nnn[nss],*]=boxcorners
                    endfor
                endfor
            endif
        endif
   ...
   'u': begin
    ...
    'g': begin
         ...
         xtickinterval= 5./3600 ; 5 RA seconds
         ytickinterval= 1./60.  ; 1 arcmin
         if (imagesize gt 6) then begin
             xtickinterval *= floor(imagesize/6)
             ytickinterval *= floor(imagesize/6)
         endif
         ...
         plot, [0,0], /nodata, xrange=xrange, yrange=yrange, $
               ...
               xtick_get=xvals, ytick_get=yvals, ticklen=ticklen, color=color, charsize=1.0, xtickinterval=xtickinterval, $
         ...
         plot, [0,0], /nodata, xrange=xrange, yrange=yrange, $
               ...
               strcompress(rahr, /remove_all)+' hours, Dec: '+strcompress(decdeg, /remove_all)+' degrees', $
               xtitle='Right Ascension [hms]', ytitle='Declination [dms]', xstyle=1, ystyle=1, position=posarray, $
               xtickn=reverse(xticlabs), ytickn=yticlabs, ticklen=ticklen, color=color, charsize=1.0, $
               xtickinterval=xtickinterval, ytickinterval=ytickinterval, /NOERASE
         ...
         oplot, m[0,*,ns].rahr,  m[0,*,ns].decdeg,psym=6, symsize=0.2, thick=2.0, color='FFFF00'XL
         xyouts, 25, 40, 'Green indicates current beam', charsize=2.0, /device, color='FFFF00'XL
         ...
         reload_image:
                                ...
                                ;Dave's Own Verson of curfull,x,y
                                ;which has the added benefit of
                                ;printing out the RA & DEC
         ...
         repeat begin
             cursor, X, Y, /nowait
             oplot, [X,X], Yfull, LINESTYLE=0, THICK=1
             oplot, Xfull, [Y,Y], LINESTYLE=0, THICK=1
             
             cursor, XX, YY, /change
             
             curRAH = floor(XX)
             curRAM = floor((XX-curRAH)*60)
             CURRAS = floor(((XX-curRAH)*60-curRAM)*60)
             curDH  = floor(YY)
             curDM  = floor((YY-curDH)*60)
             curDS  = floor(((YY-curDH)*60-curDM)*60)
             print,FORMAT='($," RA= ",I02," ",I02," ",I02,"  DEC= ",I02," ",I02," ",I02,%"\R")',curRAH,curRAM,curRAS,curDH,curDM,curDS

             oplot, [X,X], Yfull, LINESTYLE=0, THICK=1
             oplot, Xfull, [Y,Y], LINESTYLE=0, THICK=1
         endrep until (keyword_set(Change) or (!Err ne 0))
         device, SET_GRAPHICS=OldGraphics
                                ;End of Dave's Own Version of curfull,x,y
   ...
   'h': begin
         ...
         aitoff,l,b,x,y
         oplot, x,y, psym=3, symsize=0.2, thick=2.0, color='FFFF00'XL
         ...
         xyouts, 755, 15, ' GREEN', color='FFFF00'XL, /device
