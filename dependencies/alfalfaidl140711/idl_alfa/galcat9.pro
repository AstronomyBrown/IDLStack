...
;
;       January  20,3006. BK -  Added codes to the filelist names
;                              (easier to pick out zeros)
;
;       November 1, 2009, Brian Kent, NRAO - added SDSS query
;                                            functionality
;
;
;       February 1, 2010, Brian Kent, NRAO - repaired SDSS Specobj
;                                            query and AGC modify
;                                            button (line 337)
;
;       May 9, 2010.      MH -  Change AGC call to agcnorthminus1
;
;       Aug 8, 2012       DS - Calls DR9 instead of older SDSS releases
;
;----------------------------------------------------------
...
exportsdss, strmid(filelist,4,30), catstate.codes
...
;url='http://skyserver.sdss3.org/dr8/en/tools/chart/navi.asp?ra='+strcompress(double(RA_ell*15.0), /remove_all)+'&dec='+$
 ;      strcompress(double(Dec_ell), /remove_all)+'&opt=gls'

url='http://skyserver.sdss3.org/dr9/en/tools/chart/navi.asp?ra='+strcompress(double(RA_ell*15.0), /remove_all)+'&dec='+$
       strcompress(double(Dec_ell), /remove_all)+'&opt=gls'
...
if pluspos[0] eq -1 then pluspos=strpos(rdopt,'-')
...
;Store SDSS information   added Nov. 2009

catstate.objID=src.spectra[0].x_str[3]

if (catstate.sdssindex eq 1) then begin

    objID=catstate.objID
    ...
    ;Then copy to the source file
	src.spectra[0].x_str[3]=objID       ;string type
	src.spectra[0].x_str[4]=specObjID   ;string type

	src.spectra[0].x_flt[0]=petroMag_u
	src.spectra[0].x_flt[1]=petroMag_g
	src.spectra[0].x_flt[2]=petroMag_r
	src.spectra[0].x_flt[3]=petroMag_i
	src.spectra[0].x_flt[4]=petroMag_z
	src.spectra[1].x_flt[0]=petroMagErr_u
	src.spectra[1].x_flt[1]=petroMagErr_g
	src.spectra[1].x_flt[2]=petroMagErr_r
	src.spectra[1].x_flt[3]=petroMagErr_i
	src.spectra[1].x_flt[4]=petroMagErr_z
    	src.spectra[2].x_flt[0]=extinction_u
   	src.spectra[2].x_flt[1]=extinction_g
    	src.spectra[2].x_flt[2]=extinction_r
   	src.spectra[2].x_flt[3]=extinction_i
    	src.spectra[2].x_flt[4]=extinction_z
	src.spectra[0].x_flt[10]=petroR50_r
	src.spectra[0].x_flt[11]=petroR90_r
	src.spectra[0].x_flt[12]=expAB_r

	src.spectra[0].x_flt[5]=modelMag_u
	src.spectra[0].x_flt[6]=modelMag_g
	src.spectra[0].x_flt[7]=modelMag_r
	src.spectra[0].x_flt[8]=modelMag_i
	src.spectra[0].x_flt[9]=modelMag_z
	src.spectra[1].x_flt[5]=modelMagErr_u
	src.spectra[1].x_flt[6]=modelMagErr_g
	src.spectra[1].x_flt[7]=modelMagErr_r
	src.spectra[1].x_flt[8]=modelMagErr_i
	src.spectra[1].x_flt[9]=modelMagErr_z

	SpecObjstring=string(mjd, format='(a5)')+' '+$
                      string(plate, format='(a4)')+' '+$
                      string(fiberID, format='(a4)')

        src.spectra[0].x_str[5]=SpecObjstring 

endif else begin

	src.spectra[0].x_str[3]='-1'
...
catstate.sdssindex=event.index

if (catstate.sdssindex eq 1) then begin

	sloan_getobjID, objID
	catstate.objID=objID   ;string type

widget_control, catstate.filelistwidget, get_uvalue=filelist
restore, strmid(filelist[catstate.fileindex],4,30)

    objID=catstate.objID
    if (objid ne '' OR objid ne '-1') then begin
    sloan_galcat,objID,SpecObjID,$
                          petroMag_u,petroMag_g,petroMag_r,petroMag_i,petroMag_z,$
                          petroMagErr_u,petroMagErr_g,petroMagErr_r,petroMagErr_i,petroMagErr_z,$
                          extinction_u,extinction_g,extinction_r,extinction_i,extinction_z,$
...
if (catstate.sdssindex eq 0) then begin
...
restore, strmid(filelist[catstate.fileindex],4,30)
    ...
    ;Then copy to the source file
	src.spectra[0].x_str[3]='-1'   ;string type
	src.spectra[0].x_str[4]=''   ;string type

	src.spectra[0].x_flt[0]=0.0
	src.spectra[0].x_flt[1]=0.0
	src.spectra[0].x_flt[2]=0.0
	src.spectra[0].x_flt[3]=0.0
	src.spectra[0].x_flt[4]=0.0
	src.spectra[1].x_flt[0]=0.0
	src.spectra[1].x_flt[1]=0.0
	src.spectra[1].x_flt[2]=0.0
	src.spectra[1].x_flt[3]=0.0
	src.spectra[1].x_flt[4]=0.0
    	src.spectra[2].x_flt[0]=0.0
   	src.spectra[2].x_flt[1]=0.0
    	src.spectra[2].x_flt[2]=0.0
   	src.spectra[2].x_flt[3]=0.0
    	src.spectra[2].x_flt[4]=0.0
	src.spectra[0].x_flt[10]=0.0
	src.spectra[0].x_flt[11]=0.0
	src.spectra[0].x_flt[12]=0.0

	src.spectra[0].x_flt[5]=0.0
	src.spectra[0].x_flt[6]=0.0
	src.spectra[0].x_flt[7]=0.0
	src.spectra[0].x_flt[8]=0.0
	src.spectra[0].x_flt[9]=0.0

	src.spectra[1].x_flt[5]=0.0
	src.spectra[1].x_flt[6]=0.0
	src.spectra[1].x_flt[7]=0.0
	src.spectra[1].x_flt[8]=0.0
	src.spectra[1].x_flt[9]=0.0

	src.spectra[0].x_str[5]=''
...
;Setup SDSS option
     sdssoptions=['Checked no photo', 'Acquire SDSS']
     widget_control, catstate.sdsslist_id, get_uvalue=self
     option=0
     if(src.spectra[0].x_str[3] ne '' AND src.spectra[0].x_str[3] ne '-1') then option=1
     self->SetValues, sdssoptions, CurrentIndex=option
     catstate.sdssindex=option

;;Default -1 for no photometric information from SDSS
if (src.spectra[0].x_str[3] eq '' OR src.spectra[0].x_str[3] eq '-1') then begin
...
if (specObjID eq '-1') then specObjID='None'

outstring=['SDSS ObjID: '+objID+'     specObjID: '+specObjID,$
           'petroMag(err): '           +'u='+string(petroMag_u, format='(f6.2)')+'('+string(petroMagErr_u, format='(f5.2)')+')  '+$
                                   'g='+string(petroMag_g, format='(f6.2)')+'('+string(petroMagErr_g, format='(f5.2)')+')  '+$
				   'r='+string(petroMag_r, format='(f6.2)')+'('+string(petroMagErr_r, format='(f5.2)')+')  '+$
			           'i='+string(petroMag_i, format='(f6.2)')+'('+string(petroMagErr_i, format='(f5.2)')+')  '+$
				   'z='+string(petroMag_z, format='(f6.2)')+'('+string(petroMagErr_z, format='(f5.2)')+')  ',$
           '      extinct: '          +'u='+string(extinction_u, format='(f6.2)')+'  '+$
                                   '       g='+string(extinction_g, format='(f6.2)')+'  '+$
				   '       r='+string(extinction_r, format='(f6.2)')+'  '+$
                                   '       i='+string(extinction_i, format='(f6.2)')+'  '+$
                                   '       z='+string(extinction_z, format='(f6.2)')+'  ',$
           'petroR50_r: '+string(petroR50_r, format='(f6.2)')+'  '+$
           'petroR90_r: '+string(petroR90_r, format='(f6.2)')+'  '+$
           'expAB_r: '+string(expAB_r, format='(f6.2)')]

widget_control, catstate.sdsstext, set_value=outstring

endif

if (catstate.sdssindex eq 0) then begin

outstring='No SDSS information acquired for this file.'
...
catstate.fmenu=widget_button(top_menu, value=' File ')
  ...
  buttonSDSS=widget_button(catstate.fmenu, value= ' Export SDSS Catalog ', uvalue='exportsdss')
...
codebuttonbase=widget_base(codebase, /column, /align_left, /exclusive)
   ...
   modifybase=widget_base(topbase, xsize=350, ysize=290, /column, /align_left)
    ...
    buttonbase=widget_base(modifybase, xsize=200, /row)
      label=widget_label(buttonbase, value=' ')
      button=widget_button(buttonbase, value=' Save Changes ', uvalue='savechanges') 
      sdssvals=['       0','       1']
      droplist=fsc_droplist(buttonbase, value=sdssvals, uvalue='sdsslist', index=0, title='SDSS data:',xsize=400)
      catstate.sdsslist_id=droplist->getID()
...
midbase=widget_base(lowbase, xsize=605, ysize=350, /column)
catstate.mainplotwindow=widget_draw(midbase, xsize=600, ysize=310, /frame, uvalue='mainplotwindow', /keyboard_events)
catstate.sdsstext = widget_text(midbase, value = ['',''], xsize = 85, ysize = 4)
...
;Setup new SDSS option
sdssoptions=['Checked no photo', 'Acquire SDSS']
widget_control, catstate.sdsslist_id, get_uvalue=self
self->SetValues, sdssoptions, CurrentIndex=catstate.sdssindex
