...
;       August 7, 2006.  B.K. - "Spike" repair fixed for linear interpolation
;
;
;----------------------------------------------------------
...
'spectralist': begin
       ...
       if (event.index lt 7 AND event.index gt -1) then gfstate.currentlistval=event.index

       if (event.index eq 7 AND gfstate.numisolevels eq 7) then gfstate.currentlistval=event.index
       
       if (event.index eq 7 AND gfstate.numisolevels ne 7) then begin
...
if (gfstate.mode eq 'spectrameasure') then begin
   ...
   plot, gfstate.wspec[*,gfstate.currentlistval], linestyle=2, $
         ytitle='Weight', xrange=gfstate.mainxrange, yrange=[-0.2,1.5], $
...
xyouts, 15,280,'Map max flux= '+strtrim(string(gfstate.fmax, format=' (f10.2)'))+' at pix('+$
  ...
  ;print, gfstate.wspec[0:10,i]

    ;Divide weights by max
  gfstate.wspec[*,i]=gfstate.wspec[*,i]/max(gfstate.wspec[*,i])
  ;gfstate.wspec[*,i]=gfstate.wspec[*,i]/mediangrid
  
 ; print, gfstate.wspec[0:10, i]
  ;print, 'Hey testing here 2'
  ;print, gfstate.wspec[0:10,i]
...
;Gpar[4]=(coef[5]/0.4247)*dv		; HPFW in km/s
...
stn0=meanS*sqrt(nchind/4.)/rms   ; assumes han smo + boxcar to half width to rms level0,0,0
stn1=meanS/rms
stn2=A[0]/rms
stn3=totS/totSerr
...
meanSbase=mean(spec[ch1_base:ch2_base])   ; mean value over significant signal
totSerr=rms*dv*sqrt(ch2-ch1)
smofac=abs(ch1_base-ch2_base)*dv/(2.*10.)  ; assumes resolution is 10 km/s, after han
if (abs(ch1_base-ch2_base)*dv gt 200.) then smofac=10.
stn0=meanSbase*sqrt(smofac)/rms
stn1=meanS/rms
...
if (gfstate.iso[i] ne -1) then begin
  ...
  spectrum=    {name:gfstate.name[ns],			$
		...
		RA_ell:gfstate.cen_ell_ra[ns],		$
		Dec_ell:gfstate.cen_ell_dec[ns],	$
...
plot,velarr,spec,xrange=[vmin,vmax],yrange=[ymin,ymax],xstyle=1,ystyle=1,position=[0.10,0.65,0.90,0.95], $
     ytitle='Flux Density [mJy]',title=srcname+'   '+list[specnr]+' iso level',charsize=1.8
...
plot,velarr,weight,xrange=[vmin,vmax],yrange=[0.0,1.2],xstyle=1,ystyle=1,position=[0.10,0.57,0.90,0.62], $
     yticks=4,ytickv=ytickm,xtitle='Velocity [km/s]',ytitle='Weight',charsize=1.8,linestyle=1
...
h=['GALflux        ', $
   ...
   'Last update, Tuesday, March 21, 2006']
...
print, 'Initializing original GalFLUX v1.0...'
...
for i=0,n_elements(rabox)-1 do begin
   ...
   if (rabox[i] gt 24.0D) then rabox[i]=rabox[i]-24.0D
...
gfstate={baseID:0L, $         ;ID of base widget
         ...
         RA_opt:0.0D, $
         Dec_opt:0.0D, $
         correction:0, $   ;correction pointing code - not used in GALflux 1 or 2
         buttonnostatus:0L, $
         buttondetection:0L, $
         buttonprior:0L, $
         buttonmarginal:0L, $
...
tlb=widget_base(/row, title='GALflux', $
                tlb_frame_attr=1, xsize=1120, ysize=820, mbar=top_menu, $
                uvalue='tlb')
...
gfstate.fmenu=widget_button(top_menu, value=' File ', /menu)
  ...
  buttonps=widget_button(gfstate.fmenu, value=' Postscript ', uvalue='galflux_ps')
...
buttonbase=widget_base(rightbase, xsize=485, ysize=300, /column, /align_left)
...
agcinfobase=widget_base(rightbase, xsize=480, ysize=100, /column)

gfstate.agcinfo=widget_text(agcinfobase, value = ['AGC Information display'], xsize = 85, ysize = 8)
...
;outstring=['GALflux: ALFALFA spectrum measurement tool', '(c) 2006 LOVEData, Inc.', '',$
;           'Obtaining DSS2 Blue and Sloan images...']

;widget_control, gfstate.textdisplay, set_value=outstring
;galflux_dssfetch

outstring=['GALflux: ALFALFA spectrum measurement tool', '(c) 2006 LOVEData, Inc.', 'Standing by...', $
   '', 'Choose an option from the MEDIAN SKY menu to perform a median sky subtraction before you begin.', $
