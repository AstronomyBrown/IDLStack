...
;      getdss
; PURPOSE:
;       Widget based image display for DSS 2 Blue and Sloan DR7 or 8 images
;
; EXPLANATION:
;
;       Calls DSS 2 Blue via querydss.pro and SDSS via wget.  Allows
;       user to recenter image and choose image size interactively.
;       Can be called from other programs as it is stand alone.
;       Default is DR8 but if spring or fall Dec < 1 or =15, uses DR7
;
;
; CALLING SEQUENCE:
...
;
;          Jun 13, 2012   - RG/MH - modified so that DR8 is default but DR7 called
;                                   for spring or fall Dec<1 or =15.
;          
;          Aug 8, 2012 - DS - DR9 is called instead of DR8. DR7 still
;                        called for Spring or Fall Dec < 15 or = 15  
;----------------------------------------------------------
...
;xyouts, 200, 300, '(c) LOVEDATA, Inc.   Ithaca, NY', /device, charsize=2.0
  ...
  ;Get optical image from DSS 2 Blue
   ...
   ; Default is DR7 in Spring sky and DR8 in Fall sky (except dec=1 and dec=15 Fall strips, for which DR7 is used) 
