...
; yfit ==> yfit_here in lines 1169 and 1178 to prevent resizing of yfit  21mar09/RG
;
;-------------------------------------------------------------------------------------------
;
;NAME:
...
; 2mar13 .. RG: corrected typo in line 176 which reads:
;    		if (n_elements(imask) gt 0) then gindex=where(imask ne 0,ngood) ; added by rg
;           changing "gindex" to "gindx" - alerted by Carlo Giovanardi
;
;
;       
function  robfit_poly,x,y,deg,nsig=nsig,imask=imask,forcemask=forcemask,$
    ...
    ngood=npntsTot
    gindx=lindgen(ngood)
    if (n_elements(imask) gt 0) then gindx=where(imask ne 0,ngood)   ; added by rg -mod gindex ==>gindx on 2mar13
...
;                      yband=yband,measure_errors=measure_errors,/double)
           ...
           baseline.coeffs[np,nr,nj,0:norder]=coeffs
           baseline.rms[np,nr,nj]=sig
           if (sig ne 0.) then fmap[*,0,nr,0]=yfit_here
           rmsarr[nr]=sig
         endfor
;         print,rmsarr[0:10]
         fit_flag=1
