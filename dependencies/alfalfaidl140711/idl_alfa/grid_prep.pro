...
; SYNTAX:
;
; pro grid_prep, grid, Tant=Tant, savepos=savepos, masterpos=masterpos
;
; DESCRIPTION:
;
...
; RG - Entered ZA correction in Part 4 - 15nov07
; RG - Modified 1144-1153 and defntn of wmask, to account correctly for pos.status setting
; RG - Added 3 lines at beginning of PArt 3.3, setting wmask=0 for extreme values of dwrk.d
...
;***************************************************************************
pro grid_prep, grid, Tant=Tant, savepos=savepos, masterpos=masterpos, rescale=rescale, $
    batch=batch, bxcencoords=bxcencoords, bxsize=bxsize, dgdecmin=dgdecmin, $
    w_fwhm=w_fwhm, gname=gname, who=who, filename=filename
;***************************************************************************
...
if (n_elements(batch) ne 0) then begin
 print, 'Procedure currently running in batch mode'
 runmode=4
 posplus=strpos(bxcencoords,'+')
 rbxcenhours=float(strmid(bxcencoords,0,2))
 rbxcenmins=float(strmid(bxcencoords,2,2))
...
;Compute box center if mode 1 - added Feb 16, 2007
if (runmode eq 1) then begin
   dbxcen=(decmax+decmin)/2.0
   rarange=(bxsize/15.)/cosdec
   rbxcen=ramin+rarange/2.0
endif
...
If (n_elements(masterpos) eq 0) then begin
  ...
  if (n_elements(filename) eq 0) then begin
    posfind, pos, masterdir,ramin=rbxcen,decmin=dbxcen,/loadpos,gridname=bxcencoords  ;modfied April 17,2006
  endif else begin
   print, ' '
   print, '*** Using user supplied filename ***'
   print, ' '
    posfind, pos, masterdir,ramin=rbxcen,decmin=dbxcen,/loadpos, filename=filename, gridname=bxcencoords
  endelse
...
for i=0,ndrpos-1 do begin
  ...
  raphiedge=max(pos[i].rahr)*15.*!PI/180.
  raploedge=min(pos[i].rahr)*15.*!PI/180.
  decphiedge=(decpc+0.11)*!PI/180.
  decploedge=(decpc-0.11)*!PI/180.
  sin1=sin(ramind-raphiedge)
  sin2=sin(ramaxd-raploedge)
  sin3=sin(decmind-decphiedge)
  sin4=sin(decmaxd-decploedge)

; USE polarity of sin, to get around 0hrs wrap 
  if (sin1 lt 0. and sin2 gt 0. and sin3 lt 0. and sin4 gt 0.) then begin
...
if (runmode eq 1) then bxcencoords=strcompress(ramin, /remove_all)+'_'+$
                                   strcompress(ramax, /remove_all)+'_'+$
                                   strcompress(decmin, /remove_all)+'_'+$
                                   strcompress(decmax, /remove_all)

 response=tvread(filename='skycov_'+bxcencoords, /JPEG, quality=100, /nodialog)
   spawn, 'convert skycov_'+bxcencoords+'.jpg skycov_'+bxcencoords+'.gif'
   wdelete
...
; Part 3.1.1
  ...
  if (n_elements(rescale) ne 0) then begin
    ...
    for nb=0,6 do begin
      ...
      for nr=0,nrec-1 do begin
        dwrk[0,nr,nb].d=(dwrk[0,nr,nb].d-zerohalf)*av_polcor[nb]*av_bmcor[nb]
        dwrk[1,nr,nb].d=(dwrk[1,nr,nb].d+zerohalf)*(av_polcor[nb]/(2.*av_polcor[nb]-1.))*av_bmcor[nb]
      endfor
    endfor
  endif
...
; Part 3.1.3
  ...
  cal_gain=fltarr(2,8)+1.	; factors to correct for gain(ZA) - TBD; see part 4
...
;		data in given bm, pol 
  ...
  wmask=fltarr(nchn,2,nrec,nbm)+1.   ; changed from wmask=bytarr(nchn,2,nrec,nbm)+1B on 20mar09/rg

  kbig=where(abs(dwrk.d) gt 1.e10,knn)	; these two lines will catch bad pixels w/extreme values
  if (knn gt 0) then wmask(kbig)=0.	; that may have been missed by flagbb - RG/23jan2013
  print,idr,' drift ',driftname,'    bad pixels found:',knn
  ...
  for np=0,1 do begin
    for nb=0,nbm-1 do begin
      ...
      for ibb=0,nbb-1 do begin
        ...
        if (urch gt n_elements(wmask[*,0,0,0])-1) then begin
             urch=n_elements(wmask[*,0,0,0])-1
             print, 'Problem with flag - exceeds number of channels!'
        endif
...
;	LATER: DO THE CORRECTION IN FOURIER DOMAIN, RATHER THAN USING INTERPOLATE
  ...
  for nr=0,nrec-1 do begin
    ...
    heliovelproj=dwrk[0,nr,0].h.pnt.r.heliovelproj*299792.458
    heliovelused=dwrk[0,0,0].h.dop.velobsproj	;immaterial for our computations
    cenfrq_used=dwrk[0,nr,0].hf.rpfreq
    deltaf=-1420.4058*dwrk[0,nr,0].h.pnt.r.heliovelproj
...
;  d_cube=dwrk[*,*,*].d[ichstart[isec]:ichstop[isec]]
...
;        spec=d_cube[*,*,nr,near_bm]	; spectrum (2 pols: nchcube x 2) of rec nr, beam nb
        spec=dwrk[*,nr,near_bm].d[ichstart[isec]:ichstop[isec]]	; spec of (nchcube x 2 pol) 

        cont=cont_pt_wrk[*,nr,near_bm]  ; continuum point (2 element fltarr) Added by B. Kent

        raspec=dwrk[0,nr,near_bm].rahr*15.*!PI/180.	; ra of spectrum above
        decspec=dwrk[0,nr,near_bm].decdeg*!PI/180.
...
;dummy=' '
;IF ABS(total(GM)) gt 1.e6 or abs(min(gm)) GT 1.E6 or max(gm) gt 1.e6 THEN begin
;  print,'PAUSE: HIT RETURN TO CONTINUE'
;  read,dummy
;endif
...
grms=stddev(grid_map)
  ...
  delvarx,dred,dwrk,dwrk2,cont_map,mask,mc,cont_bg,cont_pt_wrk,indx
