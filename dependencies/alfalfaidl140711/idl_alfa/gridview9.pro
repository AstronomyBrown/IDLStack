...
;
;           May 14, 2010   BK -  RG reports SDSS images will not read.
;                                Used convert at command line to
;                                modify JPEG files.
;	    Sep 8, 2010    RG    changed rec 4449 from
;		map=reform((grid.d[*,0,xmin:xmax,ymin:ymax]+grid.d[*,0,xmin:xmax,ymin:ymax])/2.0)
;					to
;		map=reform((grid.d[*,0,xmin:xmax,ymin:ymax]+grid.d[*,1,xmin:xmax,ymin:ymax])/2.0)
;		(error found by RKoopman)
;
;	   December 16, 2010 - BK - modified to use skvbatch_wget perl script for images.
; 
;          Jun 13, 2012   -  MH  -  modified so that DR8 is default but DR7 called
;                                   for spring or fall Dec<1 or =15.
;
;          Aug 8, 2012  -  DS -  modified so that DR9 is default but DR7 called
;                                   for spring or fall Dec<1 or =15.
;
;----------------------------------------------------------
pro gridview_initgrid
...
pro gridview_initcommon, cat3D=cat3D
...
xdata=double(agc.rah)+double(agc.ram)/double(60.0)+double(agc.ras10)/(10.0D)/3600.0D
ydata=abs(double(agc.decd))+double(agc.decm)/double(60.0)+double(agc.decs)/3600.0D
signs=where(agc.sign eq '-')
if (signs[0] ne -1) then ydata[signs]=-ydata[signs]
...
state={baseID:0L, $                           ; ID of top base
       ...
       currentimage:0, $
       currentimage_xdata:0.0D, $
       currentimage_ydata:0.0D, $
...
delvarx, agcnorthminus1, agc, nvsscat, table3D, cattable, table2D, catalog_template
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;STARTUP;;;;;;;;;;;;;
pro gridview_startup, cat3D=cat3D
...
gridview_initcommon, cat3D=cat3D
...
state.fmenu=widget_button(top_menu, value=' File ')
 ...
 ;; buttonpsoutput=widget_button(state.fmenu, value=' Export Single Postcript ', event_pro='gridview_ps_output_gui')
...
state.scalingmenu=widget_button(top_menu, value=' Scaling ')
  buttonlinear=widget_button(state.scalingmenu, value=' Linear ', uvalue='linearscaling',/Checked_Menu) 
  buttonlog=widget_button(state.scalingmenu, value=' Logarithmic ', uvalue='logscaling',/Checked_Menu) 
  buttonhisteq=widget_button(state.scalingmenu, value=' Histogram EQ ', uvalue='histeqscaling',/Checked_Menu) 
...
;        label=widget_label(gotobase, value='     Goto channel:')
     ...
     buttoncontrolbase=widget_base(infocontrolbase, xsize=390, ysize=40, /row, /frame)
        ...
        polchoice=widget_droplist(choicebase, value=['Pol A', 'Pol B', 'Avg Pol'], $
                         event_pro='gridview_event', uvalue='polchoice')
...
;ysize=30,/row,/align_left)
 ...
 getspectrum=widget_button(spectrumbuttonbase, xsize=120, ysize=25, value='PIXEL SPECTRUM', uvalue='getspectrum')
        fluxmeasure=widget_button(spectrumbuttonbase, xsize=120, ysize=25, value='MEASURE FLUX', uvalue='fluxmeasure')
...
bothpolsbase=widget_base(spectrumbase, xsize=120, ysize=25, /row)
    button=widget_button(bothpolsbase, xsize=120, ysize=25, value='View POLS', uvalue='bothpols')
...
imagebase=widget_base(leftbase, xsize=230, ysize=360, /column)
  ...
  state.imagelabel=widget_droplist(sizetextbase, value=state.imageoptions, event_pro='gridview_event', uvalue='imageoption')
         ...
         if (agcindex[0] ne -1) then begin
          ...
          ;IF P key is hit, open the postscript dialog box
           ...
           if (event.type eq 5 AND (StrTrim(event.ch,2) eq 'p' OR StrTrim(event.ch,2) eq 'P') $
              ...
              gridview_ps_output_gui, event, xcube, ycube
          ...
          ;If H key is hit, open the drift viewing window
           ...
           if (event.type eq 5 AND (StrTrim(event.ch,2) eq 'h' OR StrTrim(event.ch,2) eq 'H') $
              ...
              gridview_fetchdrifts, xcube, ycube
          ...
          if (event.type eq 5 AND (StrTrim(event.ch,2) eq 'g' OR StrTrim(event.ch,2) eq 'G') $
               ...
               state.currentimage_xdata=xdata
               state.currentimage_ydata=ydata               
...
ra=state.currentimage_xdata
if (ra lt 0.0) then ra=ra+24

getdss, ra, state.currentimage_ydata, imagesize=imagesize
...
iagc=where(state.xdata lt ramax AND $
           state.xdata gt ramin AND $
           state.ydata lt decmax AND $
           state.ydata gt decmin AND $
...
device, decomposed=1
resultcoords=convert_coord(state.xdata[iagc], state.ydata[iagc], /data, /double, /to_device)

plots, state.xdata[iagc], state.ydata[iagc], psym=6,symsize=2, color=color, /data
...
plots, state.xdata[iagc], state.ydata[iagc], psym=4,symsize=3, color=color, /data
xyouts,resultcoords[0,*]+10, resultcoords[1,*]-10, $
             strcompress(state.agc.agcnumber[iagc], /remove_all), color=color, /device
...
plots, state.xdata[iagc], state.ydata[iagc], psym=6,symsize=symsize, color=color, /data
xyouts,resultcoords[0,*]+10, resultcoords[1,*]-10, $
             strcompress(state.agc.agcnumber[iagc], /remove_all), color=color, /device
...
if (state.checkliststatus eq 1 AND state.mapcurrent eq 'spectral') then begin
 ...
 check=findfile('checklist.txt', count=count)
    ...
    if (count eq 1) then begin
     ...
     icheck=where(xdata lt ramax AND $
           xdata gt ramin AND $
           ydata lt decmax AND $
           ydata gt decmin AND $
          ...
          if (icheck[0] ne -1) then begin
              ...
              device, decomposed=1
              plots, xdata[icheck], ydata[icheck], psym=4, symsize=2, thick=2, color='0000FF'XL, /data
              device, decomposed=0
...
pro gridview_polsettings_event, event
...
pro radecconvert, xdata, ydata, rastring, decstring
        
   radeg=(xdata/24.0)*360.0
      
   result=strtrim(adstring([radeg,ydata], 2, /truncate))
...
if (not (xregistered('gridview_jpeg_output', /noshow))) then begin
...
;---------------------------------------------------------------------
;Postscript output
pro gridview_ps_output_gui, event, xpixel, ypixel
...
if (not (xregistered('gridview_ps_output_gui', /noshow))) then begin

ps_outputtitle = strcompress('GRIDview Postscript Output')
    ...
    configbase=widget_base(ps_output_base, /column, /align_left)
       ...
       directorybase=widget_base(configbase, /row)
             ...
             state.exportdirectory=widget_text(directorybase, xsize=50, value=pwd+'/gridview.ps', $
                                               /editable, uvalue='exportdirectory')
    ...
    ps_output_export = widget_button(buttonbase, value = ' Export ', $
                uvalue = 'ps_output_export', event_pro='gridview_ps_output_event')
    cancel=widget_button(buttonbase, value=' Cancel ', uvalue='ps_output_cancel', $
                                       event_pro='gridview_ps_output_event')
    label=widget_label(buttonbase, value='    ')
             checkboxbase=widget_base(buttonbase, ysize=25, /row, /align_left, /nonexclusive)
               checkbox=widget_button(checkboxbase, value='Check this for a DSS image', $
                                                    uvalue='ps_checkbox', event_pro='gridview_ps_output_event')
    

    widget_control, ps_output_base, /realize
    xmanager, 'gridview_ps_output_gui', ps_output_base, /no_block
...
;----------------------------------------------------------------------
;Event handler for JPEG output gui
pro gridview_ps_output_event, event
...
ramin=min(xdata)-0.5*(deltaram/60.0)
ramax=max(xdata)+0.5*(deltaram/60.0)
decmin=min(ydata)-0.5/60.0
decmax=max(ydata)+0.5/60.0

xdata_opt=(ramax+ramin)/2.0
ydata_opt=(decmax+decmin)/2.0
...
if (state.ps_checkbox eq 1) then begin
  ...
  querydss, [xdata_opt*15.0, ydata_opt], optimage, header, imsize=(decmax-decmin)*60.0, survey='2b'
  widget_control, state.baseID, hourglass=0
...
contour, map, xdata,ydata, xstyle=1, ystyle=1, $
         xrange=[xdata[n_elements(xdata)-1],xdata[0]],yrange=[ydata[0], ydata[n_elements(ydata)-1]], charsize=charsize, $
...
;----------------------------------------------------------------------
pro gridview_nedgui, xdata, ydata

 common gridview_state
common gridstate
widget_control, state.baseID, hourglass=1

nedquery, xdata*15.0, ydata,10.0, numberinfo=numberinfo, string_array=string_array
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro gridview_fetchdrifts, xcube, ycube
...
if (not (xregistered('gridview_fetchdrifts', /noshow))) then begin
...
drifttitle = strcompress('Gridview Drift Viewer - Selected Pixel History')
  ...
  filelistwidget=widget_list(control_base, value=state.driftfiles[0:n_elements(driftindex)-1], xsize=36, ysize=10, uvalue='filelist', event_pro='gridview_fetchdrifts_event')
    
    state.drift_draw=widget_draw(drift_base, xsize=600, ysize=570, frame=1, $
               uvalue='drift_draw', event_pro='gridview_fetchdrifts_event', retain=2)

    label=widget_label(control_base, value=' Selected pixel is (X,Y) = ('+strcompress(xcube, /remove_all)+','+strcompress(ycube, /remove_all)+')')
    view_drift = widget_button(control_base, value = ' View  ', uvalue = 'view_drift', event_pro='gridview_fetchdrifts_event')
    drift_done = widget_button(control_base, value = ' Close ', uvalue = 'drift_done', event_pro='gridview_fetchdrifts_event')
    ...
    xmanager, 'gridview_fetchdrifts', drift_base, /no_block
    
endif

gridview_fetchdrifts_plot, -1
...
;-----------------------------------------
pro gridview_fetchdrifts_event, event
...
case uvalue of
    ...
    'filelist': begin
        ...
        gridview_fetchdrifts_plot,listnumber 
...
;-------------------------------------------
pro gridview_fetchdrifts_plot, listnumber
...
for i=0, n_elements(driftfiles)-1 do begin
    ...
    if (posindex[0] ne -1) then begin
        ...
        oplot, grid.pos[posindex].xdata, grid.pos[posindex].ydata, psym=3, color='00FF00'XL
...
oplot, grid.pos[posindex].xdata, grid.pos[posindex].ydata, psym=3, color='7F7FFF'XL
endif
...
grid=data
    ...
    if (n_params() lt 1) then begin
         print, ''
         print, 'USAGE:  gridview, grid, cat3D=cat3D'
