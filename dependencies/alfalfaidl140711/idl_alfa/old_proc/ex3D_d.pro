;-----------------------------------------------------------------------------------------------
pro disp_drift3,image,imagew,mapwindow,AXIS=axis
...
;image2=congrid(image,xs,ys)
;tvscl,image2

;stretchx=float(xs)/nx
;stretchy=float(ys)/ny
imgdisp,imagew,position=[0.08,0.05,0.95,0.50],noscale=noscale
imgdisp,image,position=[0.08,0.55,0.95,0.95],/histeq,usecongrid=usecongrid,zx=10,zy=10
...
pro get_corners3,sources,j,nra,ndec,nch,LLra,URra,LLdec,URdec,LLch,URch
      LLra=sources[j].ra-sources[j].ara*3/(2.3548*5*15/60.0)
      LLdec=sources[j].dec-sources[j].adec*3/2.3548
      URra=sources[j].ra+sources[j].ara*3/(2.3548*5*15/60.0)
      URdec=sources[j].dec+sources[j].adec*3/2.3548
...
;-----------------------------------------------------------------------------------------------
pro draw_map3,mav,flags,cont_pt,msm,agcdata,nb,nsources,sources,gain,mapwindow,nagc,xagc,yagc,iagc,VERBOSE=vrb
disp_drift3,reform(mav[0,*,nb].d),reform(flags[*,0,*,nb]),reform(cont_pt[0,*,nb])*100, $
np=0,ns=nb,mapwindow
;search_agc3,msm,agcdata,nb,nagc,xagc,yagc,iagc,VERBOSE=vrb
;draw_boxes3,msm,nsources,sources,nb,gain,VERBOSE=vrb,SINGLE='no'
end


;-----------------------------------------------------------------------------------------------
pro start_display3,gr,tav,res,n,nsources,velplane,wplane,resvelplane,matcharr,galinplane,AXIS=axis
  if (axis eq 'vel') then begin
...
pro draw_spectrum3,tav,tsm,model,x,y,n,nch,swin,AXIS=axis,TITLE=titletext
 ...
 plot,xt,tav[*,0,ra,dec],charsize=1.5,xstyle=1,xrange=[cmin,cmax],title=titletext
 if (total(model) ne 0.0) then oplot,xt,model,color=green
 arrow,ch,temp-0.04,ch,temp+0.015,color=green,/data
 plot,xt,tsm[*,0,ra,dec],charsize=1.5,xstyle=1,xrange=[cmin,cmax],/nodata
...
for n=0,nmax-1,step do begin
  ...
  display_image:
  start_display3,gr,tav,res,n,nsources,velplane,wplane,resvelplane,matcharr,galinplane,AXIS=axis
  ...
  disp_drift3,velplane,wplane,mapwindow,AXIS=axis
   ...
   if (axis eq 'dec') then begin
    ...
    if ((n ge plmin) and (n le plmax)) then begin
      ...
      if (galinplane[j] eq 1 and matcharr[j] eq 0) then begin
          colbox='FFFF00'XL
          if (sources[j].pol_flag ne 0) then colbox='BBBBFF'XL
...
;---------------------------
   'b': begin
        ...
        device, decomposed=0
        imgdisp,reform(polplot[0,*,*]),position=[0.15,0.52,0.98,0.92],$
          xrange=[xll,xur],yrange=[yll,yur],xstyle=1,ystyle=1,charsize=1.5,/histeq
        device,decomposed=1
        for i=0,nsources-1 do begin
         if (sources[i].ra gt xll and sources[i].ra lt xur and sources[i].dec gt yll and sources[i].dec lt yur) then begin
           get_corners3,sources,i,nra,ndec,nch,LLra,URra,LLdec,URdec,LLch,URch
           if ((n gt LLch) and (n lt URch)) then draw_boxes3,LLra,URra,LLdec,URdec,LLch,URch,AXIS=axis,COLOR=sources[i].color_pol
         endif
        endfor

        device,decomposed=0
        imgdisp,reform(polplot[1,*,*]),position=[0.15,0.05,0.98,0.45], $
          xrange=[xll,xur],yrange=[yll,yur],xstyle=1,ystyle=1,charsize=1.5,/histeq
        device,decomposed=1
        for i=0,nsources-1 do begin
         if (sources[i].ra gt xll and sources[i].ra lt xur and sources[i].dec gt yll and sources[i].dec lt yll) then begin
           get_corners3,sources,i,nra,ndec,nch,LLra,URra,LLdec,URdec,LLch,URch
           if ((n gt LLch) and (n lt URch)) then draw_boxes3,LLra,URra,LLdec,URdec,LLch,URch,AXIS=axis,COLOR=sources[i].color_pol
         endif 
        endfor
...
;-------------------------
   'n': begin
          ...
          for i=0,nsources-1 do begin
             if (sources[i].ra eq 0 and sources[i].dec eq 0) then continue
             if (galinplane[i] eq 1) then begin
             sources[i].cz=gr.velarr[sources[i].ch]             
             if (nagc gt 0) then begin

                 if (axis eq 'vel') then begin
                    dist_ra=abs(xagc-sources[i].ra)
                    dist_dec=abs(yagc-sources[i].dec)
                    match=where((dist_ra lt 10) and (dist_dec lt 10))
                 endif

                 if (axis eq 'ra') then begin
                    dist_ch=abs(xagc-sources[i].ch)
                    dist_dec=abs(yagc-sources[i].dec)
                    match=where((dist_ch lt 10) and (dist_dec lt 10))
                 endif

                 if (axis eq 'dec') then begin
                    dist_ch=abs(xagc-sources[i].ch)
                    dist_ra=abs(yagc-sources[i].ra)
                    match=where((dist_ra lt 10) and (dist_ch lt 10))
                 endif   

             if (match[0] ge 0) then begin
              if (n_elements(match) eq 1) then begin
;               print,'Match found with AGC galaxy'
               print,'Matching source ',strtrim(string(i),2),' with',strtrim(abs(ulong(agcdata[0,iagc[match]])),2)
               print,sources[i].ch,sources[i].ra,strtrim(string(fix(xagc[match])),2),strtrim(string(fix(yagc[match])),2)
               if (foundmatch[i] lt 1) then begin
               sources[i].comments=sources[i].comments+' AGC'+strtrim(abs(ulong(agcdata[0,iagc[match]])),2)
               sources[i].agc=strtrim(abs(ulong(agcdata[0,iagc[match]])),2)
               foundmatch[i]=1
               endif
              endif
              if (n_elements(match) gt 1) then begin
               print,'Match found with multiple AGC galaxies'
               if (axis eq 'vel') then ma=min(dist_ra,ima)
               if (axis eq 'dec') then ma=min(dist_ra,ima)
               if (axis eq 'ra') then ma=min(dist_dec,ima)
               if (foundmatch[i] lt 1) then begin
               sources[i].comments=sources[i].comments+' AGC'+strtrim(abs(ulong(agcdata[0,iagc[ima]])),2)
               sources[i].agc=strtrim(abs(ulong(agcdata[0,iagc[ima]])),2)
               foundmatch[i]=1
               endif
              endif
             endif
         endif                ;done matching with AGC
        endif 

         ;now writing to the catalogue  
          if (sources[i].pol_flag eq 0 and galinplane[i] eq 1) then begin
           if (printsource[i] lt 1) then begin
;           get_radec3,(gr.ramin+(gr.deltara/3600.)*sources[i].ra),(gr.decmin+(gr.deltadec/60.)*sources[i].dec),radec
;           printf,lun2,i,(gr.ramin+(gr.deltara/3600.)*sources[i].ra),(gr.decmin+(gr.deltadec/60.)*sources[i].dec),$
...
for i=0,nsources-1 do begin
  if (sources[i].pol_flag eq 0) then begin
           get_radec3,(gr.ramin+(gr.deltara/3600.)*sources[i].ra),(gr.decmin+(gr.deltadec/60.)*sources[i].dec),radec
           printf,lun2,i,(gr.ramin+(gr.deltara/3600.)*sources[i].ra),(gr.decmin+(gr.deltadec/60.)*sources[i].dec),$
            radec,sources[i].ch,sources[i].ra,sources[i].dec,sources[i].w,sources[i].ara,sources[i].adec,$
            sources[i].peak_flux,sources[i].int_flux,sources[i].sn,sources[i].rms,sources[i].cz,$
...
free_lun,lun2
;wdelete,mapwindow   ;Removed by BK
