...
;
function  robfit_poly,x,y,deg,nsig=nsig,imask=imask,double=double,sig=sig,gindx=gindx,$
    ...
    ngood=npntsTot
    gindx=lindgen(ngood)
    if (n_elements(imask) gt 0) then gindex=where(imask ne 0,ngood)
;   rg's lone one-liner addition ^^^
    done=0
    iter=1
	minsig=-1.
	chkiter=20
...
; recompute reads the baseline structure and recomputes baselines for all the
...
pro base_imdisplay,image,wimage,contprof,mapwindow,nj,np,nreg1,decst
...
pro base_plot_spec1,xarr,yarr,specwindow,yfit=yfit,recnr=recnr,stripnr=stripnr,$
   ...
   if (n_elements(cmask) gt 0) then begin
     ...
     oplot,xarr[badindx],yarr[badindx],psym=4,color='00FF00'XL
   endif
...
pro base_nregions,nreg1,np,nj,line1=line1,lineargs=lineargs
   ...
   print,'You can now :  - activate a box                (a)'
   print,'               - delete a box                  (d boxnr)'
   print,'               - fit the map with poly n       (f n)'
   print,'               - check baseline quality        (c recnr)'
   print,'               - go to next strip/pol map      (n or CR)'
...
for nj=n1,n2 do begin  ; WE LOOP THROUGH EACH DEC STRIP OF THE GRID
  ...
  for np=0,1 do begin    ; AND SEPARATELY FOR EACH POL
    ...
    base_imdisplay,image,wimage,contprof,mapwindow,nj,np,this_nreg,decst
...
;                      yband=yband,measure_errors=measure_errors,/double)
           ...
           coeffs=robfit_poly(xarr,spec1,norder,nsig=2.5,double=double,sig=sig,imask=mask1,$
                      gindx=gindx,ngood=ngood,bindx=bindx,nbad=nbad,$
...
;         print,rmsarr[0:10]
    ...
    'p': begin
         wdelete,mapwindow
         imagef=image-reform(fmap[*,0,*,0])
         base_imdisplay,imagef,wimage,contprof,mapwindow,nj,np,this_nreg,decst
         goto,menu
         end
