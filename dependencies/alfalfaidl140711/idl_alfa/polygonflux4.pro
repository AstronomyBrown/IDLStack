...
; EXPLANATION:
;
;       PolygonFlux extends GALflux to allow the user to mark off a simple
;       polygon (up to 100 vertices, no indentations) within which the 
;       integrated flux is to be computed.
;
...
; RESTRICTIONS:
;
;        Must have output from grid_prep, and must have deconvolution/cleaninit2 run before
;        polygonflux4.pro is compiled
;
; EXAMPLE:
;
;       @<path>/deconvolution/cleaninit2
;       .comp polygonflux4.pro
;       galflux, grid=gridbf, llx=46, lly=70, urx=53, ury=77 (or "Measure flux" 
;       within gridview)
;
; PROCEDURES USED:
...
;                              degrees declination.
;
;	Jul 3,2009	 RG	extended pointing to Fall+13,+09,+07, Spring+01
;				modified 
...
;				hpfwx=3.3
;				hpfwy=3.8
;	Aug 7, 2009	 RG	Modified "name_of_red" in pro galflux_writesource
;				to reflect true reducer name and program version
;
;	Oct 1, 2009	rg	Entered pointing corrections for Dec=+25 Spring
;				Added around line 5244:
;			coefdeg=n_elements(coefra)
;			if(coefdeg eq 1) then coefra=[coefra,0.D,0.D,0.D]
;			if(coefdeg eq 2) then coefra=[coefra,0.D,0.D]
;			if(coefdeg eq 3) then coefra=[coefra,0.D]
;
;			coefdeg=n_elements(coefdec)
;			if(coefdeg eq 1) then coefdec=[coefdec,0.D,0.D,0.D]
;			if(coefdeg eq 2) then coefdec=[coefdec,0.D,0.D]
;			if(coefdeg eq 3) then coefdec=[coefdec,0.D]
;				in order to allow fits with n<3 to pointing corrections
;
;      Dec 15, 2009     BK      Updated HI Archive spectrum read
;                               Problem encountered due to IDL upgrade
;
;	Dec 15, 2009	rg	detection code now also going to src.spectra.detcode
;
;	Feb 05, 2010	rg	Pointing correction params for Spring+29 strip entered (around rec 1870)
;
;       May 9, 2010.      MH -  Change AGC call to agcnorthminus1
;
;	Jul 14, 2010	rg	Enteredd pointing corrections for Dec=+23
;        
;	December 16, 2010 - BK - modified to use skvbatch_wget perl script for images.

;	Jan 02, 2011	rg	New pointing corrections for Spring+03
;
;       Jun 10, 2011   GLH     Modified to compute total flux in flux box as defined in gridview
;       Jun 27, 2011   GLH     Added polygon drawing in cubewindow and total flux in polygon
;       Sep 20, 2011   GLH     Modified to divide by average beam including sidelobes instead of main beam only
;       Oct 31, 2011   GLH     Incorporated making of average beam into polygonflux4
;
;			THIS FILE IS polygonflux4.pro
;
;----------------------------------------------------------
...
;------------------------------------------------------------------
;CUBE DISPLAY EVENT HANDLER

pro cube_event, event
...
;VERTEX MARK
     ...
     vertex=convert_coord(event.x, event.y, /device, /double, /to_data)
     if (vertex[0] gt cubxsize) then vertex[0]=cubxsize-1
     if (vertex[0] lt 0.0) then vertex[0]=0.0
     if (vertex[1] gt cubysize) then vertex[1]=cubysize-1
     if (vertex[1] lt 0.0) then vertex[1]=0.0

         
          device, decomposed=0

          if (event.type eq 0 AND event.press ne 4) then begin

             print,'Vertex number ',gfstate.numvertices,'     xy=',vertex
             gfstate.vertices[gfstate.numvertices,0]=round(vertex[0])
             gfstate.vertices[gfstate.numvertices,1]=round(vertex[1])
             if (gfstate.numvertices gt 0) then begin       
               x=[gfstate.vertices[gfstate.numvertices-1,0],gfstate.vertices[gfstate.numvertices,0]]
               y=[gfstate.vertices[gfstate.numvertices-1,1],gfstate.vertices[gfstate.numvertices,1]]
               device,decomposed=1
               oplot,x,y,color='00FFFF'XL
            endif
            gfstate.numvertices=gfstate.numvertices+1   
          endif
           device,decomposed=0
       end
...
;-------------------------------------------------------------------------------
;-------------------------------------------------------------------------------
pro galflux_setcoeff, coefra, coefdec

common galflux_state
common gridstate


;Current as of July 4, 2009

;UNITS in second of arc
;Correction for Dec-1 in the Spring sky - RG/4jul09
if ( gfstate.cen_ell_dec[0] ge -1.0 AND gfstate.cen_ell_dec[0] lt 2.0 AND $
    (gfstate.cen_ell_ra[0] lt 18.0 OR gfstate.cen_ell_ra[0] ge 6.0)) then begin

    ;print, 'New correction for Dec=01 Spring sky: I fitted 1st order to Ddec to avoid bad point'

      coefra=[-1.2325885D,       -0.023717337D,      -0.83839623D,      0.00042716274D]
     coefdec=[-12.218658D,         0.72335620D,      0.000000000D,      0.00000000000D]
endif

;Correction coefficients for 3rd order poly for +03 declination range of Spring Sky
; Entered by RG on 12Aug09, as computed by AM
; Recomputed by RG on 02jan11
if (gfstate.cen_ell_dec[0] gt 1.999 AND gfstate.cen_ell_dec[0] le 4.0 AND $
    gfstate.cen_ell_ra[0] le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5) then begin

;         coefra=[ -48.739163D,       65.241958D,      -27.453482D,	3.5833173D]
;        coefdec=[ -212.44660D,       228.27867D,      -79.096609D,	8.8852645D]

         coefra=[ -203.64973D,       222.06054D,      -79.989136D,	9.4112788D]
        coefdec=[ -101.63591D,       106.95807D,      -38.492363D,	4.5643885D]
...
if (gfstate.cen_ell_dec[0] gt 4.0 AND gfstate.cen_ell_dec[0] lt 16.5 AND $
    gfstate.cen_ell_ra[0]  le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5 ) then begin
...
;Correction coefficients for 3rd order poly for +23 declination range of Spring Sky - rg/14jul2010
if (gfstate.cen_ell_dec[0] gt 21.8 AND gfstate.cen_ell_dec[0] lt 24. AND $
    gfstate.cen_ell_ra[0] le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5) then begin

	COEFRA= [36081.681D,       -4757.5648D,      208.97233D,       -3.0595057D]
	COEFDEC=[103092.75D,       -13535.335D,      592.30196D,       -8.6391252D]
endif

;Correction coefficients for 3rd order poly for +25 declination range of Spring Sky
if (gfstate.cen_ell_dec[0] gt 23.999 AND gfstate.cen_ell_dec[0] lt 26. AND $
    gfstate.cen_ell_ra[0] le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5) then begin

	COEFRA= [-131115.70D,       15701.390D,      -626.64328D,       8.3335676D]
	COEFDEC=[-126392.61D,       15200.123D,      -609.22153D,       8.1371430D]
endif


;Correction coefficients for 3rd order poly for +27 declination range of Spring Sky
if (gfstate.cen_ell_dec[0] ge 25.999 AND gfstate.cen_ell_dec[0] lt 28.0 AND $
    gfstate.cen_ell_ra[0] le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5) then begin
...
;Correction coefficients for 3rd order poly for +29 declination range of Spring Sky
if (gfstate.cen_ell_dec[0] gt 27.999 AND gfstate.cen_ell_dec[0] lt 30.2 AND $
    gfstate.cen_ell_ra[0] le 17.0 AND gfstate.cen_ell_ra[0] ge 5.5) then begin

	COEFRA= [150950.00D,      -15509.986D,       531.09678D,      -6.0615043D]
	COEFDEC=[56851.944D,      -5907.1504D,       204.46148D,      -2.3578267D]
endif
...
;Correction for Dec+7 in the Fall sky
if ( gfstate.cen_ell_dec[0] gt 5.5 AND gfstate.cen_ell_dec[0] lt 8.0 AND $
    (gfstate.cen_ell_ra[0] lt 5.0 OR gfstate.cen_ell_ra[0] ge 20.0)) then begin

    ;print, 'New correction for Dec=07 Fall sky'

      coefra=[-1073.8469D,       465.43601D,      -66.719244D,       3.1753784D]
     coefdec=[-237.72132D,       111.86433D,      -17.453736D,       0.91588740D]
endif

;Correction for Dec+9 in the Fall sky
if ( gfstate.cen_ell_dec[0] ge 8.0 AND gfstate.cen_ell_dec[0] lt 10.5 AND $
    (gfstate.cen_ell_ra[0] lt 5.0 OR gfstate.cen_ell_ra[0] ge 20.0)) then begin

    ;print, 'New correction for Dec=09 Fall sky'

      coefra=[-4.2284534D,       18.539696D,      -3.8549218D,       0.20687702D]
     coefdec=[-3389.3774D,       1125.7589D,      -123.72091D,       4.5083444D]
endif

;Correction for Dec+13 in the Fall sky; a product of the rg/Am collaboration 02jul09
if ( gfstate.cen_ell_dec[0] ge 11.5 AND gfstate.cen_ell_dec[0] lt 14.5 AND $
    (gfstate.cen_ell_ra[0] lt 18.0 OR gfstate.cen_ell_ra[0] ge 6.0)) then begin

    ;print, 'New correction for Dec=13 Fall sky'

      coefra=[ -9770.6375D,       2224.8925D,      -168.21480D,       4.2228008D]
     coefdec=[ -872.01140D,       202.28782D,      -15.466440D,      0.39531804D]
endif
...
coefdec=coefra
gfstate.correction=0   ;for messaging - zero if no correction for a region, 1 if there is a correction

galflux_setcoeff, coefra, coefdec
...
if (gfstate.mode eq 'polygon') then begin
index=where(gfstate.lochhich ne 0)
    device, decomposed=1
    if (index[0] ne -1) then begin
       flag, gfstate.lochhich[index], color='0000FF'XL ; RED
   endif
   xyouts, 5,5, 'Draw Polygon', charsize=gfstate.charsize, $
...
pro galflux_boundary,xpxpk,ypxpk

  common galflux_state
  common gridstate

;  print,'galflux_boundary called'
  widget_control, gfstate.cubewindow, get_value=index 
  wset, index

  segment=fltarr(144)               ; y(x) for segment from one vertex to next
  boundary=gfstate.boundary
  vertices=gfstate.vertices
  mask=gfstate.mask
  vertices[gfstate.numvertices,*]=vertices[0,*]
  ib=0
  for i=0,gfstate.numvertices-1 do begin
    if (ib ge 500) then print,'More than 500 points in boundary!'
      xstart=vertices[i,0]
      ystart=vertices[i,1]
      xend=vertices[i+1,0]
      yend=vertices[i+1,1]
;print,xstart,ystart,'  to  ',xend,yend
      if (xstart ne xend) then begin
        inc=1
        if (xstart gt xend) then inc=-1
        for x=xstart,xend-inc,inc do begin
          segment[x]=ystart+(yend-ystart)*(x-xstart)/(xend-xstart)
          boundary[ib,0]=x
          boundary[ib,1]=round(segment[x])
;print,ib,boundary[ib,0],boundary[ib,1]
          ib=ib+1
        endfor
      endif else begin
        inc=1
        if (ystart gt yend) then inc=-1
        for y=ystart,yend-inc,inc do begin
          segment[y]=xstart+(y-ystart)*(xend-xstart)/(yend-ystart)
          boundary[ib,0]=round(segment[y])
          boundary[ib,1]=y
;print,ib,boundary[ib,0],boundary[ib,1]
          ib=ib+1
        endfor
  ...
  ibmax=ib-1
  mask[boundary[0:ibmax,0],boundary[0:ibmax,1]]=1
  xb=reform(boundary[0:ibmax,0])
  yb=reform(boundary[0:ibmax,1])
  indexp=polyfillv(xb,yb,gfstate.nbx,gfstate.nby)
  mask[indexp]=1


  gfstate.boundary=boundary
  gfstate.mask=mask
...
pro galflux_getvertices      ;  set widget control to define vertices of polygon

  common galflux_state
  common gridstate
if (gfstate.mode eq 'polygon') then begin
;  print,'galflux_getvertices called'
...
;Calculate total flux in box.  First recenter beam on box center.
center=intarr(2)
center[0]=(gfstate.nbx-1)/2.0
center[1]=(gfstate.nby-1)/2.0
...
boxstot=total(gfstate.totf[*,*])/total(shifted_beam[*,*])
print,'Total flux in box is ',boxstot/1000.0,' Jy km/s'
print,'This is without baseline removed from spectrum.'
...
;now get vertices of polygon from mouse clicks

message=['VERTEX MODE', '', 'LEFT-click the map to mark up to 100 vertices of a polygon.', $
         'GALflux will connect consecutive vertices with straight line segments.', $
         'Choose mode Ellipse when last vertex has been set...'] 

widget_control, gfstate.textdisplay, set_value=message
if (not (xregistered('galflux_getvertices',/noshow))) then begin
  xmanager,'cube',gfstate.cubewindow,/no_block
endif
endif
end
;...........................................................................
...
;.........................................................................................
;make average beam using build_beam5
pro makebeam,avgbeam
...
ylo=gfstate.lly
yhi=gfstate.ury
x=(gfstate.llx+gfstate.urx)/2
for y=ylo,yhi do begin
  build_beam5,grid,[x,y],beam
  if (y eq ylo) then begin
    avgbeam=beam
  endif else begin
    avgbeam=avgbeam+beam
  endelse
endfor
avgbeam=avgbeam/max(avgbeam)
end
...
gfstate.iso[0:6]=[0., 0.25*gfstate.fmax,100.,200.,300.,500.,1000.]
...
for i=0,gfstate.numisolevels do begin
  ...
  if i eq 0 then begin
    makebeam,avgbeam
;    restore,'avgbeam.sav'         ; 25X25 from build_beam5 averaged over decs of flux box 
    galflux_boundary,xpxpk,ypxpk  ; close polygon and set mask=1 for interior points
    index=where (gfstate.mask gt 0, where_count)
    xind=intarr(gfstate.nbx,gfstate.nby)
    yind=intarr(gfstate.nbx,gfstate.nby)
    xind=index mod gfstate.nbx
    yind=index/gfstate.nbx
    ncount=where_count
    gfstate.npixused[0]=ncount
    isomessage[0]='Isophote number 0 is the polygon: '+strcompress(ncount,/remove_all)+' pixels'
    center[0]=mean(xind)
    center[1]=mean(yind)
    print,'center=',center
    xcen=round(center[0])
    ...
    maxtest=xcen+12
    bxmax=gfstate.nbx-1
    bxmax=min([bxmax,maxtest])
    mintest=xcen-12
    bxmin=max([0,mintest])
    maxtest=ycen+12
    bymax=gfstate.nby-1
    bymax=min([bymax,maxtest])
    mintest=ycen-12
    bymin=max([0,mintest])
    abxmax=24
    abxmin=0
    abymax=24
    abymin=0
    maxtest=12+gfstate.nbx-1-xcen
    abxmax=min([abxmax,maxtest])
    mintest=12-xcen
    abxmin=max([0,mintest])
    maxtest=12+gfstate.nby-1-ycen
    abymax=min([abymax,maxtest])
    mintest=12-ycen
    abymin=max([0,mintest])
    abxmin=fix(abxmin)
    abxmax=fix(abxmax)
    abymin=fix(abymin)
    abymax=fix(abymax)
    nbx=fix(gfstate.nbx)
    nby=fix(gfstate.nby)
print,'box is ',nbx,'X',nby
print,'bxmin,bxmax,bymin,bymax = ',bxmin,bxmax,bymin,bymax
print,'abxmin,abxmax,abymin,abymax = ',abxmin,abxmax,abymin,abymax
    shifted_beam=fltarr(nbx,nby)
    shifted_beam[bxmin:bxmax,bymin:bymax]=avgbeam[abxmin:abxmax,abymin:abymax]   ; need to set center +/- 12 to avgbeam
;show the masked region
;    device,decomposed=1
    oplot,xind,yind,color='00BBFF'XL
;    device,decomposed=0
   endif
;  print,'isophote: ',i
;  print,'center=',center
...
; now must recenter beam on ellipse center
  ...
  totbeam_shifted=total(shifted_beam)
  print,'total shifted_beam = ',totbeam_shifted
...
print,'iso ',i,'beam total = ',total(shifted_beam[xind,yind])
xyouts, 15,280,'Map max flux= '+strtrim(string(gfstate.fmax, format=' (f10.2)'))+' at pix('+$
  ...
  fl=floor(center[0])
  df=center[0]-fl
  boxdiff=gfstate.rabox[fl+1]-gfstate.rabox[fl]
   if (boxdiff ge 24.0) then boxdiff=boxdiff-24.0
   if (boxdiff lt 0.0) then boxdiff=boxdiff+24.0
  gfstate.cen_ell_ra[i]=gfstate.rabox[fl]+df*(boxdiff)

  if (gfstate.cen_ell_ra[i] ge 24.0) then gfstate.cen_ell_ra[i]=gfstate.cen_ell_ra[i]-24.0
  if (gfstate.cen_ell_ra[i] lt 0.0) then gfstate.cen_ell_ra[i]=gfstate.cen_ell_ra[i]+24.0
...
spawn,'whoami',user
whowhat=''
whowhat=user+' galflux4 '+systime(0)
...
;		flux_specG_err_sys:gfstate.Gpars[ns,9]*gfstate.Gpars[ns,4]/3.,		$ assume 1/3 of rms*W50
		...
		yunits:0,			$
		x_int:lonarr(16),		$  ;Quality code stored in x_int[0]  1,2,3,4,5 or 9
		x_flt:fltarr(16),		$
		x_dbl:dblarr(8),		$
		x_str:strarr(16),		$
		comments:comments[0],		$
...
src.spectra[0].detcode[0]=gfstate.currentcode
...
common gridstate
                ...
                url='http://128.84.3.248/hiarchive/FITS/A'+filename+'.fits'

                spawn, 'wget -q -O ~/A'+ filename + ".fits '" + url + "'"                             
                
                exists=file_test('~/junk1138.fits')
     
                IF exists eq 1  THEN BEGIN
                    spawn, '/bin/rm -r ~/junk1138.fits'
                ENDIF 

                spawn, '/bin/cp ~/A'+ filename + '.fits ~/junk1138.fits'
               
                ;spawn, "sed -e 's/<VOTABLE.*>/<VOTABLE>/g' ~/A" +filename+".xml > ~/junk1138.xml"           
                spawn, 'echo $HOME', homedir
                
                archivefile=mrdfits(homedir[0]+'/junk1138.fits',1,header)
 		...
 		;	     smin=smin, smax=smax
                ...
                ;smin=min(archivefile.fluxdensity)
                 smin=min(archivefile.fluxdens)
                ;smax=max(archivefile.fluxdensity)
                 smax=max(archivefile.fluxdens)
                ;spectrum=archivefile.fluxdensity
                 spectrum=archivefile.fluxdens
                channels=n_elements(archivefile.fluxdens)
                base=archivefile.baseline
                vel=archivefile.vhelio
                
                spawn, '/bin/rm -r ~/A'+filename+'.fits'
                spawn, '/bin/rm -r ~/junk1138.fits'
...
if specnr eq 0 then begin
  oplot,gfstate.boundary[*,0],gfstate.boundary[*,1],psym=8
endif else begin
...
image=mrdfits('~/thx1138.fits', 0, header, /silent)
if (image[0] eq 0) then image=fltarr(210,210)
     ...
     'centroid_done': begin
         ...
         if (pos[0] ne -1) then begin
             ...
             if (pluspos[0] ne -1) then begin
                 ...
                 gfstate.RA_opt=hms1_hr(double(rastring))
                 gfstate.Dec_opt=dms1_deg(double(decstring))
...
print, ''
print, 'Initializing GalFLUX v4.0. Pointing corrections updated Feb. 5, 2010.'
print, ''
...
gfstate={baseID:0L, $         ;ID of base widget
         ...
         rahrimage:0.0D, $
         decdegimage:0.0D, $
         mainmode:lonarr(4), $ ;Exclusive button widget for polygon, ellipse, baseline, or fit modes
         ...
         lochhich:intarr(2), $
         spectralistvals:['Polygon', 'Quarter','100','200','300','500', '1000', 'Custom'], $  ;LISTING / BASELINE 
	 ...
	 buttoncode5:0L, $
         ...
         numvertices:0L, $
         mask:intarr(nsx,nsy), $
         vertices:intarr(100,2), $
         boundary:intarr(500,2)}

delvarx, agcnorthminus1, table3, rahr, decdeg
...
coordbase=widget_base(leftbase, xsize=485, ysize=30, /row, /frame)
   ...
   cutoutbase=widget_base(leftbase, xsize=605, ysize=300, /row)
      gfstate.cubewindow=widget_draw(cutoutbase, xsize=295, ysize=295, /frame, /button_events, $
            uvalue='cubewindow', event_pro='cube_event')
...
qualitybase=widget_base(rightbase, xsize=485, ysize=30, /row, /frame, /exclusive)
        gfstate.buttonnostatus=widget_button(qualitybase, value='0 ', uvalue='0', event_pro='galflux_codes')
        ...
        gfstate.buttoncode5=widget_button(qualitybase,   value='5:Prior- ', uvalue='5', event_pro='galflux_codes')
...
mainmodebase=widget_base(mainlabelbase, /row, /frame, /align_left)
  gfstate.mainmode[0]=widget_button(mainmodebase, value='Polygon ', uvalue='getvertices') 
  gfstate.mainmode[1]=widget_button(mainmodebase, value='Ellipse ', uvalue='ellipsecalc') 
  gfstate.mainmode[2]=widget_button(mainmodebase, value='Baseline', uvalue='baselineoption') 
  gfstate.mainmode[3]=widget_button(mainmodebase, value='Measure ', uvalue='fitmodeon') 
...
outstring=['GALflux: ALFALFA spectrum measurement tool (v.3.0)', '(c) 2006-2007 LOVEData, Inc.', 'Standing by...', $
   ...
   'Click POLYGON to mark vertices for polygon']
