...
; EXPLANATION:
;
;       Srcbrowse is a simple GUI utility tha allows users to browse
...
; EXAMPLE:
;
;       Pretty straight forward:  srcbrowse
;
; PROCEDURES USED:
...
;
;----------------------------------------------------------
pro srcbrowse_initcommon

common srcbrowse_state, catstate
end


pro srcbrowse_event, event

common srcbrowse_state
...
exportformat, filelist, catstate.codes
...
widget_control, catstate.filelistwidget, get_uvalue=filelist
restore, filelist[catstate.fileindex]
...
pro srcbrowse_externaldss, event
...
;----------------------------------------------------------------------
pro srcbrowse_nedgui, rahr, decdeg
...
if (not (xregistered('srcbrowse_nedgui', /noshow))) then begin

helptitle = strcompress('Srcbrowse NED Results')
    ...
    xmanager, 'srcbrowse_nedgui', help_base, /no_block
...
pro srcbrowse_nedgui_event, event
...
save, src, filename=filelist[catstate.fileindex]
...
if (not (xregistered('srcbrowse', /noshow))) then begin
...
;Top level base
tlb=widget_base(/column, title='ALFALFA Source Browser', $
                tlb_frame_attr=1, xsize=1040, ysize=670, mbar=top_menu, $
                uvalue='tlb')
...
externalmenu=widget_button(top_menu, value=' External ')
  button=widget_button(externalmenu, value=' SDSS Navigator ', uvalue='opennavigator')
 button=widget_button(externalmenu, value='     SkyView    ', uvalue='openskyview')
 button=widget_button(externalmenu, value='      NED       ', uvalue='openned', event_pro='srcbrowse_nedgui')
...
topbase=widget_base(tlb, xsize=900, ysize=315, /row, /frame)
   listbase=widget_base(topbase, xsize=210, ysize=330, /column)

   spawn, 'pwd', pwd
   catstate.directory=widget_text(listbase, value=pwd[0], xsize=17, /editable)
   catstate.filelistwidget=widget_list(listbase, value=filelist, xsize=31, ysize=16, uvalue=filelist, event_pro='srcbrowse_filelist')
   ...
   displaybase=widget_base(topbase, xsize=510, ysize=290, /column, /align_left)
     catstate.textdisplay = widget_text(displaybase, value = ['',''], xsize = 75, ysize = 14)
     catstate.commentsdisplay = widget_text(displaybase, value=['',''], xsize=75, ysize = 4, uvalue='commentsdisplay')

imagebase=widget_base(topbase, xsize=315, /column)

catstate.imagedisplay=widget_draw(imagebase, xsize=295, ysize=295, /frame, /button_events, uvalue='imagewindow', event_pro='srcbrowse_externaldss')
...
lowbase=widget_base(tlb, xsize=1240, ysize=580, /row)
...
;Xmanager startup
xmanager, 'srcbrowse', catstate.baseID, /no_block
