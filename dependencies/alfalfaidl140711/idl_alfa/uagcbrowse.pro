...
;       Version for laptops, e.g., for Undergrad ALFALFA workshop
;
; EXPLANATION:
...
;                as sky plot.
;
;       Modified by MPH on 08Jan04 to make general agcdir call
;             and to work on smaller laptop screens
;-
;-----------------------------------------------------------------------
...
; IF YOU WANT TO RUN ON A LAPTOP, SET WINSCALE TO SOMETHING < 1.0
winscale=0.83
...
tlb=widget_base(row=1, title='Arecibo General Catalog (Cornell EGG)', $
                tlb_frame_attr=1, xsize=1100*winscale, ysize=800*winscale, mbar=top_menu, $
                uvalue='tlb')
...
leftbase=widget_base(tlb, xsize=260*winscale, ysize=750*winscale, frame=1, /column)
  agcsearchbase=widget_base(leftbase, /row)
  label=widget_label(agcsearchbase, value='AGC ID: ')
  state.currentagcnumber=widget_text(agcsearchbase, xsize=8*winscale, /editable, value='123456', uvalue='displayagc')
  button=widget_button(agcsearchbase, value=' Display ', uvalue='displayagc')

coordsearchbase=widget_base(leftbase, /column, xsize=250*winscale, /frame)
  ...
  topcoordbase=widget_base(coordsearchbase, /row)
     ...
     state.searchra=widget_text(topcoordbase, value='12.568', xsize=7*winscale, /editable, uvalue='querposition')
     state.searchdec=widget_text(topcoordbase, value='25.025', xsize=7*winscale, /editable, uvalue='querypositon')
  bottomcoordbase=widget_base(coordsearchbase, /row)
     label=widget_label(bottomcoordbase, value='    within ')
     state.searchradius=widget_text(bottomcoordbase, value='5.0  ', xsize=7*winscale, /editable, uvalue='queryposition')
...
listbase=widget_base(leftbase, /column, xsize=250*winscale)
...
currentagcbase=widget_base(leftbase, /row, xsize=250*winscale, /base_align_left, /frame)
   labelbase=widget_base(currentagcbase, /column, /base_align_left)
    ...
    label=widget_label(labelbase,value='Morph: ')
    label=widget_label(labelbase,value='Vopt(err),ref: ')
    label=widget_label(labelbase,value='v21: ')
    label=widget_label(labelbase,value='width(err): ')
    label=widget_label(labelbase,value='HI Flux: ')
    label=widget_label(labelbase,value='rms: ')
    label=widget_label(labelbase,value='Tel/Ref: ')
   ...
   infobase=widget_base(currentagcbase, /column, /base_align_left)
    ...
    state.description=  widget_label(infobase, value='A Galaxy, 0   ')
    state.vopt=         widget_label(infobase, value='2345(80), -99 ')
    ...
    state.telcode=      widget_label(infobase, value='Arecibo, 75    ')
    state.detcode=      widget_label(infobase, value='00             ')

;------------------------------------------------------------------------
;RIGHT BASE - full Aitoff sky plot. Smaller sky plot and dss image display

rightbase=widget_base(tlb, xsize=830*winscale, ysize=630*winscale, /column)
   state.plotwindowone=widget_draw(rightbase, xsize=820*winscale, ysize=420*winscale, frame=1)

lowerbase=widget_base(rightbase, xsize=820*winscale, ysize=360*winscale, /row)
   state.plotwindowtwo=widget_draw(lowerbase, xsize=450*winscale, ysize=360*winscale, frame=1)
   state.plotwindowthree=widget_draw(lowerbase,xsize=360*winscale, ysize=360*winscale, frame=1)
...
rahr=state.agc.rah[agcindex]+state.agc.ram[agcindex]/60.0 $
              ...
              hisource=state.agc.hisource[agcindex]
...
widget_control, state.description, set_value=strcompress(description[0], /remove_all)+', '+ $
                                    strcompress(bsteintype[0], /remove_all)
widget_control, state.vopt, set_value=strcompress(vopt[0], /remove_all)+'('+$
                                      strcompress(verr[0], /remove_all)+'),'+ $
                                      strcompress(vsource[0], /remove_all)
...
widget_control, state.telcode, set_value=strcompress(telcode[0], /remove_all)+'('+ $
                                       strcompress(hisource[0], /remove_all)+')'
