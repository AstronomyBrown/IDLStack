...
;
;HISTORY:
;pjp001 23may05 - compute robust sigma for map and use 
;		         +/- clipNsig*sigma as the clipping levels for image
;				 where user now specified clipNsig rather than
;				 +/- 1.. (default is to clip to +/- 3 sigma).
;              - do clipping in call to imgdisp rather than creating
;			     a new copy of clipped_image  which is about 100+mb
;			   - for image display ,don't use /histogram equilization..
;				 the robust sigma works better.
;			   - If you wanna keep the old clipping method,
;			     fix the bug in the lower clipping level:
; 
...
;  -->Bug 		if (ncl gt 0) then clipped_image[minch]=minclip 
; 		      endfor
;				clipped_image[minchn] should be clipped_image[minch,nr]
;			  
;
...
pro display_drift,image,cmask,contprof,mapwindow,clip,np=np,ns=ns
...
window,/free,xsize=1000,ysize=600,retain=2
mapwindow=!d.window
!p.multi=[0,2,2,0,0]
loadct,1
device, decomposed=1
...
; <pjp001> clip in call to imgdisp
imgdisp,(image >clip[0])<clip[1],position=[0.05,0.32,0.89,0.95]
...
;	flagbb,dred,cont_pt,mask,pos,gausav=11,han=3,agc=1
	printTiming=1					; how long various things take
...
;
; <pjp001> lines with clipNsig below.. clipping is now
;          clipNsig * sigma of image where is computed with a robust
;          routine (outliers ignored).
clipNsig=3.
Print,'Data display will be clipped at  +/-3 sigma: OK? [y/n, def=y]'
ans=''
read,ans
if (ans eq 'n' or ans eq 'N') then begin
  print,'Enter desired n-sigma clipping level:clipNsig'
  read, clipNsig
endif

badarr=intarr(4)
print,'Do you wish to use default (d) or your own file (o) for BADBOXES? (def=d)
ans=''
read,ans
...
;   find_35,reform(m_rficor[np,*,ns].d),np,ns,badbox
   ...
   display_image:
;
; 	<pjp001> just pass the clipping levels into display_drift and
;            do the clipping all at once. Clipped_image was eating up
;      100+mb of memory for no reason.
;
;     clipped_image=image
;     for nr=0,nrec-1 do begin &$
;       maxch=where(clipped_image[*,nr] gt maxclip,ncl) &$
;       if (ncl gt 0) then clipped_image[maxch,nr]=maxclip &$
;       minch=where(clipped_image[*,nr] lt minclip,ncl) &$
;       if (ncl gt 0) then clipped_image[minch]=minclip &$ ;!! bug, missing n
;     endfor
; 
;	compute robust mean, sigma over central 1/3 of chan.
;   limit it so it goes faster, the results are the same as the whole thing.
;
; 	<pjp001>
;	Use meanrob to get the robust sigma of the central 1/3 of the 
;   freq channels. Use this for the clipping base
;
	 a=size(image)
	 nchn=a[1]
	 i1=nchn/3
	 i2=i1+ nchn/3
	 robmean=meanrob(image[i1:i2,*],sig=robsig); get sigma , ignore outliers
	 clip=[-robsig,robsig]*clipNsig		; clipping level	
     display_drift,image,contm,contprof*ufact,mapwindow,clip,np=np,ns=ns
