;+
;NAME:
;lbwbaseline - interactively baseline an LBW spectrum
;SYNTAX: lbwbaseline, lbwsrc
;ARGS:
...
while still_masking do begin
	...
	print
	print, 'Colored regions are selected.'
	print, 'There should be no emission or RFI in the marked regions.'
	print
	print, '  Left-Click to try again'
	print, '  Right-Click to accept selection'

	cursor, x, y
	button = !mouse.button
	if (button and RIGHT) ne 0 then break ; Everything's good!
...
while accepted eq FALSE do begin
	...
	legend, ['Spectrum','Baselined Region', 'Fitted Baseline'], $
	...
	; Convert to the byte value of the string '0' = 48B, '1' = 49B, etc.
	tmporder = (byte(response))[0]-48

	if strlen(response) ne 1 or tmporder lt 0 or tmporder gt omax then begin
		print, 'Only values between 0 and '+strtrim(omax,2)+' are allowed!'
		continue
	end
	order = tmporder
...
; Continuum level = average value of the spectrum in 'masked' channels
lbwsrc.continuum = mean(lbwsrc.raw[lbwsrc.mask])
...
print
print, 'Please enter any comments on baseline fitting and subtraction:'
response=''
read, response
